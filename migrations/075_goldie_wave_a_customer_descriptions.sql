...
-- This migration is deliberately not directly runnable through generic db:migrate.
-- The guarded Wave A bootstrap must validate pre-state and set this transaction-local marker.

DO $wave_a_guard$
BEGIN
  IF current_setting('shiloh.goldie_wave_a_authority', true) IS DISTINCT FROM 'PR441' THEN
    RAISE EXCEPTION 'Goldie Wave A migration requires guarded PR441 publication bootstrap';
  END IF;
END
$wave_a_guard$;
