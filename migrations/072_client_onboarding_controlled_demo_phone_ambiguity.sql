-- Repair the controlled demo identity contact trigger introduced by migration 067.
-- Do not edit migration 067: applied migrations are checksum-verified at startup.
-- PostgreSQL 42702 was raised because the PL/pgSQL local variable `normalized_phone`
-- collided with controlled_demo_identities.normalized_phone in an unqualified predicate.
-- This forward migration preserves every existing fail-closed controlled-Juvan rule
-- while using a non-conflicting local identifier for ordinary WhatsApp/mobile inserts.
...
BEGIN
  ...
  v_normalized_phone := regexp_replace(COALESCE(NEW.normalized_value, NEW.value, ''), '[^0-9]', '', 'g');
  ...
  SELECT *
   ...
   WHERE d.normalized_phone = v_normalized_phone
     AND d.active = TRUE
   FOR UPDATE;
  ...
  SELECT COUNT(DISTINCT cc.client_id)::int
   ...
   WHERE regexp_replace(COALESCE(cc.normalized_value, cc.value, ''), '[^0-9]', '', 'g') = v_normalized_phone
  ...
  IF demo_row.current_client_id IS NULL THEN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'phoneSuffix', RIGHT(v_normalized_phone, 4),
