...
Committed baseline authority is the reviewed `tests/ux-baselines/*.png.b64` text fixtures. CI decodes them to PNG only at runtime so the normal GitHub text-file write path can preserve the exact reviewed pixels.
...
4. run `node scripts/ux-baseline-codec.js encode`;
5. commit only the approved `.png.b64` fixtures together with the authorized presentation change;
6. rerun the exact-head UX gate and normal repository gates.

When a new baseline has not yet been committed, the PR workflow generates candidate `.png.b64` fixtures as an artifact and fails closed. A human/Control review must deliberately commit the accepted fixture before the gate can pass.
...
- **Permanent artifacts:** the workflow, test/config, codec, documentation, and approved baseline fixtures.
