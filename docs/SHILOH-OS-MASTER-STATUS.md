...
At the beginning of each new Shiloh OS chat: read this Master + `docs/SHILOH-OS-PROJECT-TRACKER.md` + the latest reconciliation, currently `docs/SHILOH-OS-RECONCILIATION-2026-08-19-BOOKING-UPDATE-STALE-SUPPRESSION.md`, plus `docs/SHILOH-OS-ENGINEERING-GOVERNANCE.md`, on `main`; verify applicable production/provider state; then give the four-part checkpoint: (1) authoritative current state, (2) highest-priority continuation item, (3) why it is next, (4) remaining approval/evidence/provider gate. The prior 19 August Meta approval reconciliation remains durable provider evidence at `docs/SHILOH-OS-RECONCILIATION-2026-08-19-META-BOOKING-UPDATE-APPROVAL.md`, and the Christel catalogue reconciliation remains durable evidence at `docs/SHILOH-OS-RECONCILIATION-2026-08-18-CHRISTEL-SERVICE-CATALOGUE-CORRECTION.md`; a newer current reconciliation does not erase either. Obtain explicit approval before the first new substantial controlled action. After that initial approval, continue the approved workstream automatically through ordinary engineering/deploy/verification/housekeeping boundaries. Stop only for material scope/risk expansion, contradictory authority, or an existing fail-closed human/provider/evidence gate.
...
Current accepted production application code is **PR #332 / `bd6b3963b5ba8a9518d49d9502936521a986a7bb`**, **Suppress stale booking-update retries**. The current governance baseline is **PR #317 / `f2a78bb33db212f759ac5bb72f1d832ca11cc104`**, **Enforce specialist workstream reconciliation**. Full regression CI run **#1064** passed with **695 passed / 0 failed**; Render auto-deploy **`dep-da2q3eks728c73b4jfug`** reached LIVE. Production startup preserved the approved booking-update provider state and terminally suppressed appointment #575 / audit event 674 with reason `appointment_already_ended` before any booking-update send. PR #328 / `78ece8d5bcf38aaf5d01dbaaefacde253bdef6e3` remains the accepted Christel catalogue correction lineage included in this application; PR #326 / `acefd0e3bc5a21c6e61c656ccfd8f185339c4783` remains the accepted Meta integration lineage. Later documentation-only repository heads and Render deploy identifiers do not supersede the accepted application-code baseline unless runtime code changes are verified.
...
- #332 / `bd6b3963b5ba8a9518d49d9502936521a986a7bb` — terminally suppresses stale ended booking-update retries while preserving history and future-appointment delivery; **current accepted production application code**.
...
- `shiloh_booking_update_v1` is now API **APPROVED**, `already_exists`, exact Utility/`en`, `duplicateCount=0`, quality `UNKNOWN`, and was not resubmitted. **The provider gate is closed.** Production delivery remains closed because `WHATSAPP_BOOKING_UPDATE_TEMPLATE` is unsatisfied; `WHATSAPP_BOOKING_UPDATE_ENABLED` is not independently readable and has not been reached by the genuine update-delivery path. Production / DevOps owns any separately approved configuration/enablement decision; WhatsApp / Meta Integration remains the provider-contract observer; Control & Reconciliation tracks shared authority.
...
### Booking-update stale-notification suppression and Control incident — 2026-08-19

Appointment **#575 / audit event 674** is genuine historical production queue evidence, but its appointment had already ended before booking-update activation. PR #332 therefore terminally suppresses stale service/practitioner/time/price update rows when `appointment.ends_at <= NOW()`, recording `appointment_already_ended`, preserving their audit/history, leaving them unsent, and excluding them from retry scans. Production emitted the successful suppression event for #575 / 674 at **14:30:18.812 SAST**. The row must not be released, marked sent, deleted, mutated for proof, or reused as successful delivery evidence. Genuine future update evidence must come naturally from a still-future appointment after separately approved activation.

The supplied investigation observed `attempt_count=27`, but an additional old-code retry occurred at **14:25:13 SAST** before PR #332 deployed. The exact post-deploy numeric counter is therefore not asserted; suppression itself does not increment it. The sanctioned Render read-only SQL connector failed SSL/TLS negotiation before executing SQL, so no write-capable workaround was used.
...
For service/practitioner/date-time/price changes, Shiloh records a durable audit-event-idempotent outbox item only after the canonical mutation succeeds. PR #332 extends that durable contract with terminal `suppressed` handling: a pending/failed booking-update row whose appointment has already ended is suppressed before provider/configuration work and rechecked before send claim, with reason `appointment_already_ended`; it is not sent or deleted and cannot return to the retry scanner. Genuine future appointment changes remain deliverable through `shiloh_booking_update_v1` once the separate production gates are intentionally satisfied. Admin cancellation continues using `shiloh_cancellation_confirmation_v1` and is not stale-suppressed by this rule.
...
Current provider truth: cancellation confirmation is APPROVED; `shiloh_booking_update_v1` is **APPROVED, exact and duplicate-free**. The provider gate is closed, but ordinary update delivery remains blocked because `WHATSAPP_BOOKING_UPDATE_TEMPLATE` is unsatisfied and the separate `WHATSAPP_BOOKING_UPDATE_ENABLED` state is not independently established. Appointment #575 / audit event 674 is now terminally suppressed and must not be released or used as delivery evidence. Do not claim delivery or change production configuration without separate approval.
...
- `shiloh_booking_update_v1` provider approval is **closed/APPROVED**; its remaining gate is intentional production template-name configuration plus explicit delivery enablement under Production / DevOps control. PR #332 has closed the stale-appointment retry hazard; #575 / 674 is terminally suppressed and cannot be released or used as delivery evidence.
...
- Production application code is the accepted PR #332 / `bd6b3963b5ba8a9518d49d9502936521a986a7bb` baseline; governance baseline remains PR #317 / `f2a78bb33db212f759ac5bb72f1d832ca11cc104`. CI #1064 passed 695 / 0 and Render auto-deploy `dep-da2q3eks728c73b4jfug` reached LIVE. Later documentation-only commits/deploys do not supersede that application behaviour unless runtime code changes are verified.
...
- Customer-change confirmation architecture is live; cancellation template is APPROVED; `shiloh_booking_update_v1` is **APPROVED / exact / duplicate-free**, but production delivery remains closed on `WHATSAPP_BOOKING_UPDATE_TEMPLATE` and separately gated enablement. #575 / audit event 674 is terminally suppressed with `appointment_already_ended`, not sent, and no longer eligible as a future delivery journey. Genuine update-delivery evidence must come from a still-future appointment after separately approved activation.
...
**Authoritative current state:** PR #332 / `bd6b3963b5ba8a9518d49d9502936521a986a7bb` is the accepted regression-green production application baseline; CI #1064 passed 695/0 and Render `dep-da2q3eks728c73b4jfug` is LIVE. The Christel reviewed catalogue correction remains complete and must not be redone: #27 inactive/unmapped with seven histories preserved; #34/#65 retained at 120/50; reviewed totals 60/90/90. Christel, Abigail and Marietjie each finalize only their own appointments; JP finalizes none. PR #320/#322 menu presentation and JP's #318 booking entitlement remain unchanged. `shiloh_booking_update_v1` is provider **APPROVED**, exact Utility/`en`, duplicate-free and not resubmitted; production delivery remains fail-closed because `WHATSAPP_BOOKING_UPDATE_TEMPLATE` is unsatisfied and the enablement gate is not independently established. #575 / audit event 674 has been terminally suppressed for `appointment_already_ended` without a WhatsApp send. Governance baseline #317 remains authoritative.

**Highest-priority next item:** route a separately approved **Production / DevOps booking-update configuration decision**. Before any configuration action, re-read the current provider/configuration state, explicitly determine the intended `WHATSAPP_BOOKING_UPDATE_TEMPLATE` value and `WHATSAPP_BOOKING_UPDATE_ENABLED` state, and preserve the #332 stale-appointment suppression contract. Do not use #575 as a release candidate or delivery test.

**Why next:** the Meta provider gate and the stale-retry safety defect are both closed. The remaining booking-update work is therefore a production configuration/operational decision; successful delivery evidence must arise naturally from a genuine change to a still-future appointment after activation, not from further provider submission or a manufactured journey.

**Remaining gates:** no Meta approval gate remains for `shiloh_booking_update_v1`. Production configuration/enablement still requires separate explicit approval; #575 must remain suppressed and must not be sent, marked sent or mutated by Control. Historical attendance/#558, Goldie description approval, Google Business Profile access, and other genuine lifecycle evidence retain their existing gates and owners.