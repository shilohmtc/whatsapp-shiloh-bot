...
Current accepted **application** code is **PR #395 / `485ed97d8812fc291c71493dd1bb652b5da42f05`**, **Fix practitioner calendar conflict classification**, built on and preserving PR #388 guarded Juvan booking cleanup, PR #387 provider-log redaction, PR #385 Meta-template reconciliation, PR #383/#384 booking-confirmation-v2 activation/evidence, PR #382 reschedule-template activation and the accepted #380 Couples Massage lineage.

- GitHub CI run **#1228** for PR #395 passed; focused pre-merge coverage passed **30/30** and the full pre-merge regression passed **856/856**. Reconciliation PR #396 CI run **#1230** also passed.
- PR #395 production deploy **`dep-da4a75lckfvc738ghpmg`** reached **LIVE** on exact merge SHA `485ed97d8812fc291c71493dd1bb652b5da42f05` in confirmed workspace **My Workspace**.
- Documentation-only PR #396 merged as `9bd3251122475d9b2b36fffa2408b8e95d442bfc` and auto-deployed as **`dep-da4abn9t0dsc73adlld0`**, preserving #395 application behavior unchanged. Repeated current `/health` request logs on 2026-08-22 returned HTTP 200. Later documentation-only reconciliation commits do not supersede #395 application authority; current Render convergence must still be verified at continuation time.
...
- Newer production evidence on 2026-08-22 records three WhatsApp onboarding failures with PostgreSQL **42702**, `column reference "normalized_phone" is ambiguous`, in `clientIdentityOnboarding.js` `completeOnboarding()` → `processActiveSession()`. This is a distinct CRM & Identity defect and does not invalidate #395 Calendar verification. Earlier clean log windows remain valid only for the bounded verification windows in which they were observed.
- Provider credential rotation is complete. The post-revocation verification deploy `dep-da47v6n40ujc73d1qeug` reached LIVE on documentation-only PR #389 / `ae3825925277205512a4db0d9e13964fb3e79ea5`; the dedicated Meta system user `Shiloh` owns the final production-only token with exactly `whatsapp_business_management` and `whatsapp_business_messaging`; all former generic `Employee` tokens are revoked. Root/health and Meta provider checks passed in that verified window, which contained zero errors and zero credential values. Newer unrelated CRM onboarding errors do not reopen credential rotation.
...
- No genuine Couples Massage appointment, companion identity, Calendar event, booking decision, cancellation, Juvan reset/re-registration or handset journey was manufactured merely to prove #380 or #395.
...
Post-revocation Render deploy `dep-da47v6n40ujc73d1qeug` reached LIVE on the then-current documentation-only `main`, PR #389 / `ae3825925277205512a4db0d9e13964fb3e79ea5`. Root and health probes returned 200. Booking-update, cancellation, staff-finalization and booking-confirmation provider checks remained APPROVED, including configured booking-confirmation v2. That verified window contained zero errors and zero Authorization, Bearer, Meta-token-like or `WHATSAPP_TOKEN` values. No real customer message or booking was created. Newer unrelated CRM onboarding errors do not reopen credential rotation.
...
**Current production defect:** on 2026-08-22 three WhatsApp onboarding completions failed with PostgreSQL `42702` / `column reference "normalized_phone" is ambiguous` in the current `clientIdentityOnboarding.js` completion path. This is newer runtime evidence and is owned by CRM & Identity for bounded investigation and repair, with Production / DevOps observing runtime/deployment evidence and Control tracking reconciliation. No root cause is established by this control record, and no CRM record, onboarding identity or WhatsApp journey should be manufactured merely to reproduce it.
...
**Authoritative current application state:** PR #395 / `485ed97d8812fc291c71493dd1bb652b5da42f05`; CI #1228 passed, focused tests passed 30/30 and full regression passed 856/856. PR #395 deploy `dep-da4a75lckfvc738ghpmg` reached LIVE on exact merge SHA. Documentation-only PR #396 / `9bd3251122475d9b2b36fffa2408b8e95d442bfc` passed CI #1230 and auto-deployed as `dep-da4abn9t0dsc73adlld0`. Repeated current `/health` requests observed on 2026-08-22 returned HTTP 200. Later documentation-only reconciliation commits do not supersede #395 application authority; verify current Render convergence on continuation. PR #387 credential-log redaction, PR #385 Meta reconciliation, booking-confirmation v2 selection/evidence, #382 reschedule activation, #380 Couples Massage and all standing fail-closed invariants remain preserved.
...
**Current CRM defect:** 2026-08-22 Render evidence records three WhatsApp onboarding failures with PostgreSQL `42702`, `column reference "normalized_phone" is ambiguous`, in `clientIdentityOnboarding.js`. CRM & Identity owns the next bounded investigation/repair; Production / DevOps observes runtime/deployment evidence; Control tracks reconciliation. This Master does not assert a root cause and does not authorize a corrective production mutation.
