...
At the beginning of each new Shiloh OS chat: read this Master + `docs/SHILOH-OS-PROJECT-TRACKER.md` + the latest reconciliation, currently `docs/SHILOH-OS-RECONCILIATION-2026-08-19-BOOKING-UPDATE-PRODUCTION-ACTIVATION.md`, plus `docs/SHILOH-OS-ENGINEERING-GOVERNANCE.md`, on `main`; verify applicable production/provider state; then give the four-part checkpoint: (1) authoritative current state, (2) highest-priority continuation item, (3) why it is next, (4) remaining approval/evidence/provider gate. The prior stale-suppression reconciliation remains durable safety evidence at `docs/SHILOH-OS-RECONCILIATION-2026-08-19-BOOKING-UPDATE-STALE-SUPPRESSION.md`, the 19 August Meta approval reconciliation remains durable provider evidence at `docs/SHILOH-OS-RECONCILIATION-2026-08-19-META-BOOKING-UPDATE-APPROVAL.md`, and the Christel catalogue reconciliation remains durable evidence at `docs/SHILOH-OS-RECONCILIATION-2026-08-18-CHRISTEL-SERVICE-CATALOGUE-CORRECTION.md`; a newer current reconciliation does not erase any of them. Obtain explicit approval before the first new substantial controlled action. After that initial approval, continue the approved workstream automatically through ordinary engineering/deploy/verification/housekeeping boundaries. Stop only for material scope/risk expansion, contradictory authority, or an existing fail-closed human/provider/evidence gate.
...
Current accepted production application code is **PR #332 / `bd6b3963b5ba8a9518d49d9502936521a986a7bb`**, **Suppress stale booking-update retries**. The current governance baseline is **PR #317 / `f2a78bb33db212f759ac5bb72f1d832ca11cc104`**, **Enforce specialist workstream reconciliation**. Full regression CI run **#1064** passed with **695 passed / 0 failed**. PR #332's Render auto-deploy **`dep-da2q3eks728c73b4jfug`** reached LIVE and production terminally suppressed appointment #575 / audit event 674 with reason `appointment_already_ended` before any booking-update send. PR #328 / `78ece8d5bcf38aaf5d01dbaaefacde253bdef6e3` remains the accepted Christel catalogue correction lineage included in this application; PR #326 / `acefd0e3bc5a21c6e61c656ccfd8f185339c4783` remains the accepted Meta integration lineage.

Booking-update production activation is now **LIVE / ENABLED** on the same accepted runtime-code baseline. GitHub `main` remained **`29cf4ebc249b8b85d66a1616a26e35bd9e9739a0`** during activation; Production / DevOps set `WHATSAPP_BOOKING_UPDATE_TEMPLATE=shiloh_booking_update_v1` and `WHATSAPP_BOOKING_UPDATE_ENABLED=true` in one Render environment merge update. API-triggered activation deploy **`dep-da2qovs9v7es73cqlrr0`** reached LIVE at **2026-08-19 15:16:16 SAST**. Post-restart health returned HTTP 200, Google Calendar provider health passed, and startup booking-update verification reported `submitted=false`, `reason=already_exists`, `providerStatus=APPROVED`. No Meta template resubmission or unexpected booking-update send occurred during startup. Later documentation-only repository heads and Render deploy identifiers do not supersede PR #332 as the accepted application-code baseline unless runtime code changes are independently verified.
...
- `shiloh_booking_update_v1` — **APPROVED / UTILITY / `en`**, exact contract, `duplicateCount=0`, quality `UNKNOWN`, `already_exists`, not resubmitted; production template configuration and explicit enablement are now LIVE.
...
The supplied stale-suppression investigation observed `attempt_count=27`, but an additional old-code retry occurred at **14:25:13 SAST** before PR #332 deployed. The exact post-deploy numeric counter is therefore not asserted; suppression itself does not increment it. The sanctioned Render read-only SQL connector failed SSL/TLS negotiation before executing SQL in that earlier checkpoint, so no write-capable workaround was used.
...
Fresh activation-startup evidence again reports **`Google Calendar provider health check passed`**.
...
- Production application code remains the accepted PR #332 / `bd6b3963b5ba8a9518d49d9502936521a986a7bb` baseline; governance baseline remains PR #317 / `f2a78bb33db212f759ac5bb72f1d832ca11cc104`. CI #1064 passed 695 / 0. Production booking-update activation ran on docs-only main `29cf4ebc...` as API deploy `dep-da2qovs9v7es73cqlrr0`, LIVE and healthy; later documentation-only commits/deploys do not supersede PR #332 application behaviour unless runtime code changes are verified.
...
**Authoritative current state:** PR #332 / `bd6b3963b5ba8a9518d49d9502936521a986a7bb` remains the accepted regression-green production application baseline; CI #1064 passed 695/0. Booking-update production configuration is **LIVE / ENABLED**: exact template `shiloh_booking_update_v1`, explicit enablement true, provider APPROVED/existing/exact/duplicate-free/no resubmission, activation deploy `dep-da2qovs9v7es73cqlrr0` healthy LIVE. #575 / audit event 674 remains terminally suppressed for `appointment_already_ended` with `sent_at=null` and can never serve as delivery evidence. Governance baseline #317 remains authoritative.

**Highest-priority next item:** resume the already-active historical attendance final review by re-querying current CRM state and presenting only safely routable unresolved appointments to the responsible practitioner Admin for explicit human determination. Booking-update successful delivery evidence stays open but passive until a genuine still-future appointment change occurs naturally.

**Why next:** booking-update provider approval, stale-row safety repair, production template configuration and explicit enablement are complete. No implementation action is justified merely to create delivery evidence. Historical attendance is already an active human-truth workstream and can continue without manufacturing a booking-update journey.

**Remaining gates:** successful booking-update customer delivery evidence requires a natural still-future appointment change; #575 remains historical-only. Historical attendance/#558, Goldie description approval, Google Business Profile access, and other genuine lifecycle evidence retain their existing gates and owners.