...
At the beginning of each new Shiloh OS chat: read this Master + `docs/SHILOH-OS-PROJECT-TRACKER.md` + the latest reconciliation, currently `docs/SHILOH-OS-RECONCILIATION-2026-08-19-CLIENT-WELCOME-DIAGNOSTIC.md`, plus `docs/SHILOH-OS-ENGINEERING-GOVERNANCE.md`, on `main`; verify applicable production/provider state; then give the four-part checkpoint: (1) authoritative current state, (2) highest-priority continuation item, (3) why it is next, (4) remaining approval/evidence/provider gate. The prior booking-update activation reconciliation remains durable production/configuration evidence at `docs/SHILOH-OS-RECONCILIATION-2026-08-19-BOOKING-UPDATE-PRODUCTION-ACTIVATION.md`, the stale-suppression reconciliation remains durable safety evidence at `docs/SHILOH-OS-RECONCILIATION-2026-08-19-BOOKING-UPDATE-STALE-SUPPRESSION.md`, the 19 August Meta approval reconciliation remains durable provider evidence at `docs/SHILOH-OS-RECONCILIATION-2026-08-19-META-BOOKING-UPDATE-APPROVAL.md`, and the Christel catalogue reconciliation remains durable evidence at `docs/SHILOH-OS-RECONCILIATION-2026-08-18-CHRISTEL-SERVICE-CATALOGUE-CORRECTION.md`; a newer current reconciliation does not erase any of them. Obtain explicit approval before the first new substantial controlled action. After that initial approval, continue the approved workstream automatically through ordinary engineering/deploy/verification/housekeeping boundaries. Stop only for material scope/risk expansion, contradictory authority, or an existing fail-closed human/provider/evidence gate.
...
- #335 / `59469d6670cb116a5be20ebc3ab682d4f36ad717` — adds the authenticated SELECT-only client universal-welcome diagnostic with ambiguity-safe canonical identity reporting; CI #1070 700/0, Render `dep-da2ttdcs728c73b99mbg` LIVE; **current accepted production application code**.
...
Fresh #335 startup evidence again reports **`Google Calendar provider health check passed`**.
...
The universal welcome routing contract from PR #312 remains accepted: a greeting is offered to `processClientTransitionWelcome()` before ordinary client identity/discovery handling. The durable `v2` implementation uses phone-level `client_whatsapp_welcome_deliveries` state plus the canonical-client `whatsapp_universal_welcome_v2_sent_at` marker. A genuine prior `v2` delivery allows subsequent greetings to fall through to the normal client menu.

During the controlled Juvan journey on 19 August, a real handset `Hi` produced the normal three-button client home menu instead of the universal welcome. Render independently recorded the incoming text and a three-button outbound response. Older successful sends for the same masked number were located before the `v2` rollout, but no corresponding successful post-rollout `v2` welcome send was located before this greeting. The previous transition implementation used a different marker (`whatsapp_transition_welcome_sent_at`), so older pre-v2 evidence is not itself proof of `v2` delivery.

This observation is **not yet a proven root cause**. It is expected if the durable `v2` ledger reflects a genuine prior delivery; otherwise it indicates an inconsistent client-specific welcome state or an additional handler/state defect that must be traced before repair.

PR #335 / `59469d6670cb116a5be20ebc3ab682d4f36ad717` adds a narrowly scoped authenticated read-only diagnostic at `/audit-read/client-welcome/status`. It is protected by `auditReadAuth`, executes exactly two parameterized `SELECT` queries, returns only last-four phone correlation plus `v2` ledger state, canonical identity status/count and unique-only marker state, and fails closed on ambiguous identity. It returns no full phone, client ID/name or contact data and provides no write/reset/booking path. CI #1070 passed 700/0; Render deploy `dep-da2ttdcs728c73b99mbg` is LIVE and healthy.

The direct Render Postgres read-only connector still fails before SQL execution with SSL/TLS negotiation errors. PostgreSQL external inbound traffic remains blocked and must not be opened merely to obtain this evidence.

**Current gate:** invoke #335 using the **user-confirmed Juvan WhatsApp number** while keeping `AUDIT_READ_TOKEN` private. Do not infer Juvan's canonical identity, registration status or marker truth from handset/log behavior. Do not send another handset greeting merely for proof before this read.

**Repair authority:** none has been granted. The user approved only the read-only diagnostic. Any ledger deletion/reset, client mutation, CRM identity correction, appointment action or proof-triggering outbound send after a proven defect is a separate material boundary requiring explicit approval.
...
Registered/legacy Book appointment routing, eligible-practitioner ordering, booking/cancellation action-button parity, practitioner directory, category ordering/count polish, SQT BioMicroneedling virtual-family presentation, Admin booking/reschedule, 24-hour presentation, typed-time repair, canonical/Goldie-aware client lookup, provisional-client reservation and cleanup remain completed. The universal welcome **routing code contract** remains accepted, but the newer Juvan client-specific `v2` state is an active read-only diagnostic gate and must not be dismissed under older “do not redo” wording.
...
- Client universal-welcome routing remains accepted, but the 19 August Juvan `v2` persisted state is **waiting authenticated read-only evidence** through PR #335. Do not infer identity/state, weaken database networking, reset welcome state, or send another greeting merely for proof. Any repair is a separate approval boundary.
...
- Production application code is the accepted PR #335 / `59469d6670cb116a5be20ebc3ab682d4f36ad717` baseline; governance baseline remains PR #317 / `f2a78bb33db212f759ac5bb72f1d832ca11cc104`. CI #1070 passed 700 / 0; Render auto-deploy `dep-da2ttdcs728c73b99mbg` is LIVE and healthy. PR #335 adds only the authenticated SELECT-only client-welcome diagnostic; PR #332 stale-suppression, #328 catalogue and #326 Meta behavior remain included.
- Universal welcome routing from #312 remains accepted, but a real 19 August Juvan greeting reached the normal three-button home menu without located post-v2 outbound welcome evidence. The client-specific ledger/marker truth is not yet read. Use #335 with the user-confirmed number and private audit token before classifying or repairing.
...
**Authoritative current state:** PR #335 / `59469d6670cb116a5be20ebc3ab682d4f36ad717` is the accepted regression-green production application baseline; CI #1070 passed 700/0 and Render `dep-da2ttdcs728c73b99mbg` is healthy LIVE. The new diagnostic is authenticated and SELECT-only. Booking-update production remains **LIVE / ENABLED** and provider APPROVED/existing/exact/duplicate-free/no resubmission. #575 / audit event 674 remains terminally suppressed historical evidence. Governance baseline #317 remains authoritative.

**Highest-priority next item:** complete the authenticated read-only `v2` welcome-state diagnostic for the user-confirmed Juvan WhatsApp number before continuing the handset journey or recommending any state repair.

**Why next:** the observed home-menu response is expected only if `v2` was already durably delivered. #335 now provides the minimum evidence needed to distinguish expected once-only suppression from stale persisted state or a deeper handler defect without opening database networking or mutating CRM/booking truth.

**Remaining gates:** the read requires the user-confirmed phone and private `AUDIT_READ_TOKEN`; the Render SQL connector remains blocked by SSL/TLS negotiation. No welcome-state repair is authorized under the read-only approval. Existing booking-update natural-delivery, historical attendance/#558, Goldie description, GBP and natural lifecycle evidence gates remain unchanged.
