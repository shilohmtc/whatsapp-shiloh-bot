...
Current accepted production application code is **PR #344 / `3cf8dbce36c58d9f52c07951481d171d28d61539`**, **Verify booking confirmation v2 provider contract after submission**. PR #343 / `311ce80030b4ef7600d55b8a73e895729d22b595` freezes `shiloh_booking_confirmation_v2` as a non-sendable exact contract and reuses canonical appointment handlers; PR #344 adds sanitized immediate provider contract read-back. Full CI #1093 passed **716 / 0** for #343 and full CI #1095 passed **717 / 0** for #344. Render `dep-da382vu7bikc738s3te0` reached LIVE before provider submission. Exactly one controlled submission then occurred on deploy `dep-da383btg1s2s73d19cn0`; immediate Meta read-back established **PENDING / UTILITY / en / exact / duplicateCount=0**. The one-shot provisioning flag was restored to `false`, and deploy `dep-da383pdg1s2s73d1a40g` reached LIVE with `shiloh_booking_confirmation_v1` still configured **APPROVED / UTILITY / already_exists** and no v2 provisioning event. The current governance baseline remains **PR #340 / `aeb4e35361c34413d4310b1846c7043642a31cd2`**, **Require copy-ready specialist handoffs**. PR #337 welcome routing, PR #335 authenticated diagnostics, PR #332 stale booking-update suppression, PR #328 Christel catalogue correction, PR #326 centralized Meta integration and PR #334 booking-update production activation remain preserved in this runtime lineage.

Booking-update production activation remains **LIVE / ENABLED** under the previously reconciled #334 state. Production is still configured with `WHATSAPP_BOOKING_UPDATE_TEMPLATE=shiloh_booking_update_v1` and `WHATSAPP_BOOKING_UPDATE_ENABLED=true`; the activation deploy `dep-da2qovs9v7es73cqlrr0` remains durable evidence of that boundary. Booking confirmation production remains on `shiloh_booking_confirmation_v1`; v2 is provider PENDING, `sendable=false`, `ready=false`, and not activated.
...
- #344 / `3cf8dbce36c58d9f52c07951481d171d28d61539` — adds sanitized immediate provider component/status/duplicate-count read-back after controlled v2 submission; CI #1095 717/0; **current accepted production application code**.
...
- `shiloh_booking_confirmation_v1` — **APPROVED / UTILITY / configured live fallback**.
- `shiloh_booking_confirmation_v2` — **PENDING / UTILITY / `en` / exact / `duplicateCount=0`** after exactly one controlled 20 August submission; production inactive/non-sendable.
...
- `shiloh_booking_confirmation_v1` remains the production booking-confirmation selector and was reconfirmed on the 20 August flag-off restart as **APPROVED / UTILITY / already_exists**, `submitted=false`.
- `shiloh_booking_confirmation_v2` was submitted exactly once after #343/#344 full CI. Immediate provider read-back is **PENDING / UTILITY / `en` / exact / `duplicateCount=0`**. Its static header, exact five-variable body, footer and three QUICK_REPLY buttons match the frozen contract. It remains `sendable=false`, `ready=false`; `META_BOOKING_CONFIRMATION_V2_PROVISION_ON_START=false`; production still selects v1. **Do not activate or resubmit while PENDING.**
...
Fresh production restart evidence continues to preserve the PR #302 fail-closed provider guard. Real WhatsApp evidence previously verified booking **#570** end to end: practitioner changed to **Christel**, the **Google Calendar event was updated**, and Linda Dr / Sports Massage — Package Session / 2026/08/21 14:30–15:20 / R0.00 were preserved. Do not mutate #570 again merely for proof.
...
PR #303 covers successful Admin changes to service, practitioner, date/time, booked price, and cancellation. New bookings continue using the established approved `shiloh_booking_confirmation_v1` production path while v2 remains provider PENDING and non-sendable.
...
- `shiloh_booking_confirmation_v2` is **provider PENDING / exact / duplicate-free / production inactive**. The one controlled submission is complete; do not resubmit. `shiloh_booking_confirmation_v1` remains the live configured fallback. Activation is blocked until a future read-only provider check establishes APPROVED + exact + duplicate-free, followed by a separately approved production activation change.
...
- Production application code is the accepted PR #344 / `3cf8dbce36c58d9f52c07951481d171d28d61539` baseline. PR #343/#344 CI gates passed 716/0 and 717/0. Governance baseline remains PR #340 / `aeb4e35361c34413d4310b1846c7043642a31cd2`.
- Booking confirmation v2 is **PENDING / Utility / en / exact / duplicateCount=0**, submitted exactly once. Its provider components match the frozen contract. It remains `sendable=false`, the one-shot provisioning flag is `false`, and production continues using approved v1.
...
**Authoritative current state:** PR #344 / `3cf8dbce36c58d9f52c07951481d171d28d61539` is the regression-green production application baseline; CI #1095 passed 717/0. Governance PR #340 remains authoritative. `shiloh_booking_confirmation_v2` exists at Meta after exactly one controlled submission and immediate read-back is **PENDING / UTILITY / en / exact / duplicate-free**. It is not active or sendable; the one-shot provisioning flag is off; `shiloh_booking_confirmation_v1` remains configured, APPROVED and live. Existing booking-update, welcome, catalogue, attendance and other durable authority is unchanged.

**Highest-priority next item:** no v2 implementation or activation action is currently authorized. WhatsApp / Meta Integration remains the monitoring owner for the external Meta approval state. A future read-only recheck may establish whether status has changed; do not resubmit. Only after APPROVED + exact + duplicate-free evidence may a separately approved Production / DevOps activation decision be routed.

**Why next:** local contract/handler engineering, regression, full CI, deployment, exactly one provider submission, immediate provider contract verification and submission-gate shutdown are complete. The unresolved dependency is external Meta review, not another code or provider-submission task.

**Remaining gates:** booking confirmation v2 Meta approval; separate future activation approval/code/config; and natural handset evidence only after an activated v2 is used by a genuine booking. Do not manufacture a booking. Existing booking-update natural-delivery, historical attendance/#558, Goldie description, GBP and other natural lifecycle evidence gates remain unchanged.
