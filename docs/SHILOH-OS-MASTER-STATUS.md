...
- Production runtime code baseline: `1dd589904114726b7696f34bed9ce5800a7c6aa2` after PR #201 (`Correct MediHeel ownership to Christel`). Render deploy `dep-d9vdjgnavr4c73agbv4g` reached `live` on that exact runtime commit.
...
- PR #197 is a presentation-only booking-entry refinement: the four stable `client_family_*` IDs are unchanged, while the visible family rows now read `Beauty & Aesthetics`, `Massage Treatments`, `Lymphatic Drainage`, and `Elim MediHeel Pedicures`, with action-oriented `View ... treatments` descriptions.
- **Authoritative human clinic evidence on 2026-08-14 states that MediHeel treatments are currently bookable with Christel only.** This supersedes the temporary Marietjie assumption introduced during PR #199 investigation.
- PR #199 remains useful historical evidence that the two active Medi-Heel catalogue services already existed and that the zero-row WhatsApp result was a staff-service ownership problem rather than an absent catalogue. However, its Marietjie ownership assignment is superseded by PR #201 and must not be reused as current truth.
- **PR #201 corrected the live family and CRM ownership contract to Christel-only.** The client family rule now exposes `Elim MediHeel Pedicures • Christel`; eligible practitioner resolution is Christel-only; the family scope is deliberately limited to MediHeel/Elim service names rather than broad `Pedicures & Foot Care`, so unrelated pedicure/aesthetic mappings are not reassigned.
- PR #201 startup repair is fail-closed and idempotent: exactly one active Christel practitioner must exist, and matching active MediHeel/Elim service mappings are reassigned to Christel before the HTTP listener opens. The Render deploy reached `live`, proving the corrected startup path completed successfully.
- No PR #201 change creates, activates, renames or reprices any service.
- Direct Render Postgres read remains tooling-limited by the known SSL/TLS connector failure. Do not use that limitation to infer CRM rows; current clinic human evidence plus the live startup contract are authoritative for MediHeel ownership.
...
- Real Dummy Test evidence on 2026-08-14 exposed sequential deterministic client defects: language interception, natural-family routing fallthrough, and MediHeel ownership drift. Each proven deterministic defect must become a permanent regression before repair where feasible.
- Explicit clinic/operator evidence overrides an earlier engineering inference about practitioner eligibility. When that happens, correct both runtime and project ledger rather than preserving the incorrect inference as operational truth.
- The prior classification of the Elim MediHeel zero-row result as unknown catalogue truth is superseded: catalogue evidence proved the treatments exist. The temporary PR #199 Marietjie assignment is also superseded: current clinic evidence and PR #201 establish Christel-only ownership.
- Do not seed duplicate MediHeel services.
- PR #197 code/CI/deploy evidence proves the booking-entry menu copy is live in production, but real WhatsApp presentation acceptance should only be marked complete after it is naturally observed through Meta transport.
...
- PR #197: regression-first branch commit `ed9e64437c1c858a790d506f8e8851a8f1ae8110` failed as intended for the new menu copy. A later unsafe whole-file overwrite was caught by CI and corrected before merge. Final head `810992a9240d5f46f3b59955035b4c629bb2bf84` passed CI #532. Squash merge `61151ba771ffd90213cc8947ae28f661e152f768`; GitHub `main` CI #533 passed; Render deploy `dep-d9vd7n3ncjis738r6nq0` reached live.
- PR #199: regression-only commit `1144d92441de1563b2ff9b430edae87672569673` failed CI #536 before implementation; final head `b362c1391cb7d17f27ba9da239274604963f619e` passed CI #541; squash merge `0562ca279304ab8fd92b7bc2cb69bea758ba6f3e`. Its catalogue/root-cause evidence is retained, but its Marietjie ownership assignment is superseded.
- **PR #201:** regression-only commit `b28919fb99a38474bcb13a05536d88f7af24a658` failed CI #545 before implementation. After implementation, CI #550 exposed two stale assertions still encoding the superseded Marietjie/broad-pedicure rule; those tests were corrected to the narrower clinic contract. Final head `0e43c29e1dcd31079a7f85db7ee0fa163e5db5d1` passed CI #552. Squash merge `1dd589904114726b7696f34bed9ce5800a7c6aa2`; Render deploy `dep-d9vdjgnavr4c73agbv4g` reached live.

## Product-Critical Gate

🔵 **Client Perspective Testing remains product-critical. The immediate next acceptance check is Elim MediHeel family presentation after the Christel-only correction; once that passes, resume the genuine held-booking approval lifecycle.**
...
The natural Lymphatic family routing gate is complete. The Elim MediHeel ownership defect is now corrected to the clinic-authoritative Christel-only rule and production-live, but its real Meta/WhatsApp list presentation still needs one minimal human acceptance check.

For Elim MediHeel Pedicures, do not create or seed services. Existing authoritative Medi-Heel catalogue rows should now be visible through the Christel-only family route if their active/client-bookable state remains valid.
...
- 🔵 Elim MediHeel Pedicures ownership is **Christel-only by authoritative clinic evidence and PR #201**, production-live. One real WhatsApp family-list presentation check remains before marking the client surface accepted.
...
PR #201 is merged, CI-green and production-live. Current authoritative rule: **MediHeel treatments are bookable with Christel only.** **Next send/select `Elim MediHeel Pedicures` once from CRM Dummy Test and confirm the real WhatsApp treatment list appears with Christel as the owner/practitioner path.** Do not seed treatments and do not recreate #561. If the family list renders correctly, mark C1-MEDIHEEL real-accepted and continue the same Dummy Test client journey into a genuine future booking. At booking completion, stop at pending approval before anyone decides; capture Dummy Test’s held/not-confirmed wording, verify slot exclusion, and confirm JP/Jean-Pierre alone receives actionable Approve/Decline. Then JP approval should unlock final client confirmation plus both Calendar mirrors. Test JP decline separately later.
