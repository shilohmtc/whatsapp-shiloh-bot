...
- PR #409 production deploy **`dep-da4njtlckfvc73cmnk0g`** reached **LIVE** on exact merge SHA `696a2c669a3de7b21f8119f0786c707974c30ffd` at `2026-08-22T10:30:46.679185Z`.
- The production startup completed the established bootstrap/migration verification through migration 072, the service listener started successfully, and an inspected production request returned HTTP 200.
...
- The #409 startup inspection also observed a separate Google Calendar provider-health warning reporting insufficient `calendar.events.readonly` permission. It did not block the #409 deploy or application startup and is not caused by the Admin presentation repair. Treat it as separate time-bounded provider evidence to re-verify under the Google Calendar / Production owner before Calendar-specific action; do not broaden or reopen #409 for it.
- Later documentation-only reconciliation commits and their automatic Render deploys are time-bounded evidence only and do not supersede PR #409 as application behavior; exact GitHub/Render convergence must be re-verified at continuation time.
...
## Google Calendar — 🟢 DURABLE RULES / 🟠 CURRENT PROVIDER PERMISSION WARNING
...
During #409 production startup verification, the provider-health path logged a warning reporting insufficient `calendar.events.readonly` permission. The application still reached LIVE and an inspected request returned HTTP 200, so this is not evidence that the #409 presentation repair failed. It is, however, newer provider-health evidence than the prior clean Calendar window and must be re-verified by the Google Calendar / Production owner before any Calendar-specific action is claimed healthy. Existing fail-closed booking safeguards remain authoritative.

#362 separately verified deterministic cleanup of archived Dummy Test mirrors; post-cleanup 2026 searches returned zero Dummy Test events on shared, Abigail, Marietjie and primary/Christel surfaces. #380 previously passed the Google Calendar provider health probe; the later #409 warning does not rewrite that historical evidence.
...
**Authoritative current application state:** PR #409 / `696a2c669a3de7b21f8119f0786c707974c30ffd`; workflow run 32567694026 passed on Node 24.14.1, full non-mutating regression passed 868/868 with 0 failed/0 skipped and four focused fallback/routing/startup cases. Exact Render deploy `dep-da4njtlckfvc73cmnk0g` reached LIVE at `2026-08-22T10:30:46.679185Z`; established bootstrap/migration verification completed through migration 072, the application listener started, and an inspected production request returned HTTP 200. #409 changes only the legacy unrecognized Admin presentation boundary and preserves guarded commands, authorization, auditing and active booking/date routing. PR #399 remains durable authority for the normalized-phone ambiguity repair; PR #395 remains durable authority for practitioner Calendar semantics. Later documentation-only reconciliation commits/deploys do not supersede #409 application behavior; verify current Render convergence on continuation.
