...
Current accepted production application code is **PR #350 / `bb26eb62a719f84cbe0471aa54530e71cb104da9`**, **Route canonical Juvan bookings to JP approval**. Full CI #1120 passed **744 / 744, 0 failed**. Render auto-deploy `dep-da39p0bm8hqs739jldtg` reached LIVE and production startup applied/checksum-verified migration `065_juvan_botha_jp_booking_approval.sql` before opening the listener. The production preflight authoritatively resolved canonical CRM client **845 / Juvan Botha** with `activeNameCount=1`, one canonical WhatsApp/mobile identity and `sharedActiveContactCount=0`, and exact approver **Jean-Pierre admin ID 4** with WhatsApp configured. The new booking-approval policy is keyed by canonical client ID, not a display-name pattern. Existing CRM Dummy Test → JP approval and ordinary practitioner approval behavior are preserved. No Juvan booking or WhatsApp journey was manufactured for proof.

PR #348 / `de624ccdd51834841f2a94355afa48d13d2b0f3d` remains the accepted live-v1 delivery-polish lineage: the exact configured `shiloh_booking_confirmation_v1` path suppresses the four redundant post-confirmation supplemental groups while preserving the approved Meta v1 contract, canonical Calendar/manage-booking/My appointments handlers and non-template fallback behavior. The #350 restart reconfirmed v1 `submitted=false`, `reason=already_exists`, configured `shiloh_booking_confirmation_v1`, provider **APPROVED / UTILITY**. Google Calendar provider health also passed.

PR #346 / `3cdaca8950dc4ad8d808e575334848ec428490dd` and PR #347 / `a7bf72cd05f06bb566d3cc1810363c5fe09c51a0` remain accepted dark-deploy lineage for practitioner-approved client reschedules. Startup under #350 reconfirmed migration `064_client_reschedule_practitioner_approval.sql` checksum-verified/already applied with the request table/indexes present, while `featureEnabled=false`, `approvalTemplateConfigured=false` and `declinedTemplateConfigured=false`. That feature remains non-active and must not be inferred enabled from code/schema presence.
...
- #348 / `de624ccdd51834841f2a94355afa48d13d2b0f3d` — suppresses four redundant automatic post-confirmation supplemental groups for exact live v1 while preserving provider contract and canonical handlers; CI #1116 737/0, Render `dep-da397up42hec73aplkkg` LIVE; **accepted live-v1 delivery-polish lineage**.
- #350 / `bb26eb62a719f84cbe0471aa54530e71cb104da9` — binds production-verified canonical client 845 / Juvan Botha to exact Jean-Pierre admin ID 4 for client-booking approval using checksum migration 065 and ID-keyed database/application guards; CI #1120 744/0, Render `dep-da39p0bm8hqs739jldtg` LIVE; **current accepted production application code**.
...
### Client booking approval — Juvan Botha and Dummy Test → JP

PR #350 adds one explicit canonical-client approval exception without changing practitioner selection or Admin entitlement. Canonical CRM client **845 / Juvan Botha** is production-verified and bound by durable policy `juvan_botha_jp_booking_approval` to exact **Jean-Pierre admin ID 4**. Migration 065 and the application layer independently require the canonical Juvan identity to remain active/unique, to retain at least one WhatsApp/mobile identity, to share no such identity with another active client, and to retain the exact guarded JP `business_admin` / `all_business` / `all_services` / WhatsApp contract.

Runtime matching is by persisted `client_id=845`, never a name pattern. A different client merely named Juvan Botha, a duplicate active name, shared contact identity, missing policy, or JP contract drift fails closed. The held booking remains a normal availability conflict until JP decides. JP approval unlocks the existing customer confirmation path; JP decline preserves audit/history, cancels the held appointment and releases both Calendar mirrors. Existing OS CRM Dummy Test → JP approval remains unchanged. Ordinary Abigail/Christel/Marietjie and all other client approval behavior remains unchanged.

Do not broaden this policy by name, change the canonical client ID without authoritative CRM evidence, create a duplicate Juvan identity, or manufacture a booking merely to demonstrate the approval path.
...
This feature is **not active**. Production verification under #350 reports `featureEnabled=false`, `approvalTemplateConfigured=false`, and `declinedTemplateConfigured=false`. Code/schema presence is not authorization. Do not enable the feature or configure/submit its transport templates without a separate controlled Meta/configuration unit and explicit activation authority.
...
### Juvan Botha → Jean-Pierre booking approval — 2026-08-20

PR #350 / `bb26eb62a719f84cbe0471aa54530e71cb104da9` makes one explicit canonical-client booking approval rule production-live. The sanctioned Render Postgres read connector remained blocked at SSL/TLS negotiation, so migration 065 plus startup read-back served as the fail-closed production identity preflight. The new instance proved exact canonical client **845 / Juvan Botha**, active exact-name count 1, one canonical WhatsApp/mobile identity, zero shared-active-client identity count, and exact approver **Jean-Pierre admin ID 4** with WhatsApp configured before opening the listener.

The durable policy is keyed by `client_id=845`; name is only a verification backstop. The database trigger and application service independently revalidate the policy. A missing policy, duplicate active Juvan name, shared contact identity, client drift, or JP contract drift blocks the special path. Existing Dummy Test → JP and ordinary practitioner/observer approval semantics are unchanged. CI #1120 passed 744/744; Render `dep-da39p0bm8hqs739jldtg` is LIVE/healthy. No Juvan booking or WhatsApp approval event was manufactured for evidence.
...
The supplied stale-suppression investigation observed `attempt_count=27`, but an additional old-code retry occurred at **14:25:13 SAST** before PR #332 deployed. The exact post-deploy numeric counter is therefore not asserted; suppression itself does not increment it. The sanctioned Render read-only SQL connector failed SSL/TLS negotiation before executing SQL in that earlier checkpoint, again during the #335 client-welcome diagnostic checkpoint, and again before #350 Juvan policy implementation, so no write-capable workaround was used.
...
Fresh #350 production restart evidence again reports **`Google Calendar provider health check passed`**. Real WhatsApp evidence previously verified booking **#570** end to end: practitioner changed to **Christel**, the **Google Calendar event was updated**, and Linda Dr / Sports Massage — Package Session / 2026/08/21 14:30–15:20 / R0.00 were preserved. Do not mutate #570 again merely for proof.
...
During the controlled Juvan journey on 19 August, an earlier real `Hi` incorrectly produced the normal three-button client home menu. PR #335's authenticated SELECT-only diagnostic was then invoked using the user-confirmed number and returned sanitized evidence for suffix `1564`: `ledger.exists=false`, `ledger.sentAt=null`, `canonicalIdentity.status=unique`, `activeClientCount=1`, and a resolved canonical marker with `exists=false`, `sentAt=null`. That proved the earlier home-menu response was not legitimate once-only suppression. The unique canonical match is the only identity fact established by that earlier read; it does not imply consent, guardian status or unrelated CRM attributes. PR #350 later established the exact production canonical row for its separate approval policy as client 845.
...
Universal welcome routing is repaired and handset-proven through PR #337. Registered/legacy Book appointment routing, eligible-practitioner ordering, booking/cancellation action-button parity, practitioner directory, category ordering/count polish, SQT BioMicroneedling virtual-family presentation, Admin booking/reschedule, 24-hour presentation, typed-time repair, canonical/Goldie-aware client lookup, provisional-client reservation and cleanup remain completed. The Juvan JP approval rule is now production-live through #350; do not recreate a Juvan booking merely for proof.
...
- Canonical CRM client **845 / Juvan Botha** is explicitly bound to exact Jean-Pierre admin ID 4 for client-booking approval through #350. Runtime routing is by persisted client ID and fails closed on identity/contact/JP drift. Existing Dummy Test approval is unchanged. Do not broaden the rule by name or manufacture a booking for proof.
...
- Production application code is the accepted PR #350 / `bb26eb62a719f84cbe0471aa54530e71cb104da9` baseline. CI #1120 passed 744/744; Render `dep-da39p0bm8hqs739jldtg` is LIVE/healthy. Governance baseline remains PR #340 / `aeb4e35361c34413d4310b1846c7043642a31cd2`.
- Canonical CRM client **845 / Juvan Botha** is production-verified with one canonical WhatsApp/mobile identity and zero shared-active-client identity conflicts, and is bound by ID-keyed policy to exact **Jean-Pierre admin ID 4** for client-booking approval. Existing Dummy Test → JP and all unrelated client approval behavior remain unchanged.
...
- Google Calendar OAuth/provider health is **🟢 VERIFIED HEALTHY**; #350 restart reconfirmed the health probe.
...
**Authoritative current state:** PR #350 / `bb26eb62a719f84cbe0471aa54530e71cb104da9` is the regression-green production application baseline; CI #1120 passed 744/744 and Render `dep-da39p0bm8hqs739jldtg` is LIVE/healthy. Canonical CRM client 845 / Juvan Botha is uniquely production-verified and routed by an ID-keyed policy to exact Jean-Pierre admin ID 4 for booking approval. Live v1 remains configured/APPROVED with #348 polish. PR #346/#347 remain dark/default-off. Governance PR #340 remains authoritative. `shiloh_booking_confirmation_v2` remains last-authoritatively **PENDING / UTILITY / en / exact / duplicate-free**, non-sendable and inactive.

**Highest-priority next item:** the Juvan approval unit is complete. WhatsApp / Meta Integration returns as the monitoring owner for the external v2 Meta approval state. Perform only a read-only provider recheck when approval may have changed; do not resubmit. If and only if v2 becomes APPROVED while exact and duplicate-free, reconcile that evidence and route a separate controlled Production / DevOps activation under PR #340.

**Why next:** the client-specific Juvan rule is implemented, full-regression green, merged, canonical identity/JP policy verified in production, and live without manufacturing a booking. The interrupted v2 engineering/submission work is also locally complete; the remaining v2 dependency is external Meta review rather than further Booking & Admin UX work.

**Remaining gates:** booking confirmation v2 Meta APPROVED status; separate future v2 activation approval/code/configuration; natural handset evidence only from legitimate future journeys; separately gated reschedule-approval Meta/config activation; existing booking-update natural delivery, CRM Dummy Test genuine reset evidence, historical attendance/#558, Goldie description, GBP and other natural lifecycle gates remain unchanged.
