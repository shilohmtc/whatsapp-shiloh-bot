...
Current accepted **application** code is **PR #399 / `26ace1027e10f40e41d0f5d981e72f4a55a972c6`**, **Fix CRM onboarding normalized phone ambiguity**, built on and preserving PR #395 practitioner Calendar conflict classification, PR #388 guarded Juvan booking cleanup, PR #387 provider-log redaction, PR #385 Meta-template reconciliation, PR #383/#384 booking-confirmation-v2 activation/evidence, PR #382 reschedule-template activation and the accepted #380 Couples Massage lineage.

- PR #399 GitHub Actions CI run **#1236** passed on Node **24.14.1**. The full non-mutating regression passed **860/860**, with **0 failed** and **0 skipped**. Four focused regression cases prove the historical migration 067 identifier collision, migration 072's collision removal without guard weakening, ordinary onboarding canonical contact/duplicate handling, and startup migration ordering.
- PR #399 production deploy **`dep-da4me5qd0e5s73bobfm0`** reached **LIVE** on exact merge SHA `26ace1027e10f40e41d0f5d981e72f4a55a972c6` at `2026-08-22T09:09:08Z`.
- Startup logged migration **072_client_onboarding_controlled_demo_phone_ambiguity.sql** with `applied=true` and `checksumVerified=true`; historical migrations 065–068 remained `checksumVerified=true`. The controlled Juvan identity remained **BOUND**, the assigned-practitioner Primary + Jean-Pierre Backup + first-decision-wins approval contract remained verified, `Shiloh started` was logged, and repeated `/health` requests returned HTTP 200.
- A bounded post-cutover Render search from `2026-08-22T09:09:08Z` through `09:09:55Z` returned zero `normalized_phone` matches. This is bounded error-absence evidence, not a manufactured end-to-end onboarding journey.
- The repaired root cause was migration 067's `client_contacts` BEFORE INSERT/UPDATE trigger function declaring local `normalized_phone` and evaluating `WHERE d.normalized_phone = normalized_phone`; PostgreSQL treated the unqualified identifier as ambiguous and raised `42702` before ordinary onboarding could complete. Applied migration 067 remains checksum-authoritative and was not edited. Forward migration 072 replaces only `guard_and_rebind_controlled_demo_contact()` using `v_normalized_phone` while preserving canonical contact ownership, duplicate/contact conflict guards, controlled Juvan ownership, the existing-bound-client guard, WhatsApp-onboarding-only unbound rebind, atomic approval-policy pointer rebind, controlled identity audit, and fail-closed approval-policy pointer consistency.
- PR #395 / `485ed97d8812fc291c71493dd1bb652b5da42f05` remains the durable authority for practitioner Google Calendar conflict classification. Its CI #1228, focused **30/30**, full **856/856**, and production deploy `dep-da4a75lckfvc738ghpmg` remain accepted evidence for that bounded rule. #399 does not alter it.
- PR #395's durable Calendar rule is bounded: an unrelated practitioner's practitioner-Calendar event does not, by itself, make the assigned practitioner unavailable. Shared/clinic-wide events and events on the relevant assigned practitioner's Calendar remain blocking. Canonical CRM conflicts, `calendar_blocks`, schedules/exceptions, clinic hours, Google Calendar provider health and all existing booking/approval fail-closed guards remain authoritative.
- Later documentation-only reconciliation commits and their automatic Render deploys are time-bounded evidence only and do not supersede PR #399 as application authority; exact GitHub/Render convergence must be re-verified at continuation time.
- Provider credential rotation is complete. The post-revocation verification deploy `dep-da47v6n40ujc73d1qeug` reached LIVE on documentation-only PR #389 / `ae3825925277205512a4db0d9e13964fb3e79ea5`; the dedicated Meta system user `Shiloh` owns the final production-only token with exactly `whatsapp_business_management` and `whatsapp_business_messaging`; all former generic `Employee` tokens are revoked. Root/health and Meta provider checks passed in that verified window, which contained zero errors and zero credential values. The separately repaired CRM onboarding incident does not reopen credential rotation.
...
- Startup reverified migrations 065/066/067/068 as checksum-valid and Juvan controlled identity as **BOUND** to the current canonical pointer, presently client **845**, display `Juvan Botha`, controlled phone suffix **1564**, Jean-Pierre admin **4**. PR #399 additionally verified migration 072 applied/checksum-valid while preserving that bound state.
...
- No genuine CRM onboarding, CRM identity, Couples Massage appointment, companion identity, Calendar event, booking decision, cancellation, Juvan reset/re-registration or WhatsApp provider mutation was manufactured merely to prove #399, #380 or #395.
...
Post-revocation Render deploy `dep-da47v6n40ujc73d1qeug` reached LIVE on the then-current documentation-only `main`, PR #389 / `ae3825925277205512a4db0d9e13964fb3e79ea5`. Root and health probes returned 200. Booking-update, cancellation, staff-finalization and booking-confirmation provider checks remained APPROVED, including configured booking-confirmation v2. That verified window contained zero errors and zero Authorization, Bearer, Meta-token-like or `WHATSAPP_TOKEN` values. No real customer message or booking was created. The later CRM onboarding ambiguity incident was repaired by #399 and does not reopen credential rotation.
...
Authoritative foundation reconciliation: `docs/SHILOH-OS-RECONCILIATION-2026-08-20-CONTROLLED-JUVAN-DEMO-IDENTITY.md`. Current Booking/Admin approval semantics are reconciled separately in the Booking/Admin reconciliation and remain preserved through #380 and #399.
...
Production migration bootstrap deliberately started from the already-persisted and previously verified Juvan approval-policy/client relationship, then required one exact phone and zero shared-active-client conflict. New-instance startup after #380 reverified, and PR #399 startup preserved:
...
No genuine Juvan reset/re-registration occurred merely for #380 or #399 proof. Production remains BOUND to the current pointer until a separately authorized real device lifecycle. The live foundation is proven; the future reset→registration transition is intentionally not claimed as handset-proven here.
...
#399 does not alter #395's Calendar classification semantics.
...
No Couples Massage or synthetic Calendar event was manufactured merely for proof. Canonical Shiloh CRM/appointment state remains authoritative; Google Calendar remains a synchronized provider/mirror.
...
**CRM onboarding normalized-phone ambiguity — 🟢 VERIFIED LIVE / COMPLETE:** on 2026-08-22 three WhatsApp onboarding completions failed with PostgreSQL `42702` / `column reference "normalized_phone" is ambiguous` while `completeOnboarding()` performed the canonical `client_contacts` insert. Root cause is now authoritative: migration 067's `client_contacts` trigger function declared local `normalized_phone` and compared `d.normalized_phone = normalized_phone`, creating a PL/pgSQL identifier collision. Applied migration 067 was not edited. PR #399 added forward migration 072, replacing only `guard_and_rebind_controlled_demo_contact()` with `v_normalized_phone` while preserving every standing controlled-identity/contact guard. CI #1236 passed 860/860 plus four focused cases. Exact Render deploy `dep-da4me5qd0e5s73bobfm0` reached LIVE on `26ace1027e10f40e41d0f5d981e72f4a55a972c6`; migration 072 applied/checksum-verified, 065–068 remained checksum-valid, Juvan remained BOUND, repeated `/health` returned 200, and the bounded post-cutover window through `09:09:55Z` contained zero `normalized_phone` matches. No real CRM identity, onboarding journey, booking, Calendar event or WhatsApp provider mutation was manufactured for proof.

**Imported contact-book provenance audit — 🟠 BLOCKED ON CURRENT PRODUCTION EVIDENCE:** new business evidence establishes that imported personal address-book contacts are not automatically verified Shiloh client identity. Current 2026-08-22 production aggregate/provenance evidence remains unknown because the sanctioned Render read-only PostgreSQL connector fails before SQL execution with `FATAL: SSL/TLS required` / unexpected EOF. The production database `shiloh-memory` remains `available`, and active-connection metrics stayed non-zero during the same window, so this is treated as a connector/external TLS-client capability gate rather than proof of a general database outage. There is no connected authenticated Render CLI, SSH, shell, psql, secret-read or alternate SQL path. Historical 2026-08-16 counts must not be reused as current evidence. The Linda exact-phone trace is blocked because no exact phone anchor has been supplied; no display-name or `Linda Dr` lookup is authorized. CRM & Identity remains the owning workstream; Production & DevOps is evidence observer only. Control authorizes only an existing-credential, TLS-authenticated, explicitly read-only PostgreSQL observation path with no Render environment/configuration change, application code change or redeploy. Once available, Production & DevOps must execute the existing 11-query CRM audit evidence pack and return masked evidence to CRM & Identity for audit closure. Until then, the audit is not complete and no bulk merge/archive/rename/rewrite remediation is authorized.
...
**Authoritative current application state:** PR #399 / `26ace1027e10f40e41d0f5d981e72f4a55a972c6`; CI #1236 passed on Node 24.14.1, full non-mutating regression passed 860/860 with 0 failed/0 skipped and four focused regression cases. Exact Render deploy `dep-da4me5qd0e5s73bobfm0` reached LIVE at `2026-08-22T09:09:08Z`; migration 072 applied/checksum-verified, historical migrations 065–068 remained checksum-valid, Juvan remained BOUND, the Primary + JP Backup / first-decision-wins contract remained verified, `Shiloh started` was logged, repeated `/health` returned 200, and the bounded post-cutover search through `09:09:55Z` returned zero `normalized_phone` matches. Later documentation-only reconciliation commits/deploys do not supersede #399 application authority; verify current Render convergence on continuation. PR #395 remains durable authority for its practitioner Calendar semantics. PR #387 credential-log redaction, PR #385 Meta reconciliation, booking-confirmation v2 selection/evidence, #382 reschedule activation, #380 Couples Massage and all standing fail-closed invariants remain preserved.

**Imported contact-book identity audit:** **BLOCKED / current production evidence not obtained.** The sanctioned Render read-only PostgreSQL connector fails before SQL with `FATAL: SSL/TLS required` / unexpected EOF while the database remains available with live connections. No alternate connected authenticated SQL/shell/CLI path exists. Do not reuse 2026-08-16 counts. Linda remains untraced because no exact-phone anchor has been supplied; no display-name matching. CRM & Identity remains owner; Production & DevOps is evidence observer only. The authorized recovery is an existing-credential TLS-authenticated, explicitly read-only PostgreSQL client path without Render configuration changes, application changes or redeploy. Once available, 40 runs the existing 11-query evidence pack and returns masked evidence to 20 for audit closure. Dated blocker authority is `docs/SHILOH-OS-RECONCILIATION-2026-08-22-IMPORTED-CONTACT-BOOK-DB-EVIDENCE-GATE.md`.

**Practitioner Calendar classification:** #395 is verified live and do-not-redo. An unrelated practitioner's Calendar event does not, by itself, block the assigned practitioner. Shared/clinic-wide events and the relevant assigned practitioner's Calendar remain blocking, with provider-health and canonical conflict rules preserved. Dated authority is `docs/SHILOH-OS-RECONCILIATION-2026-08-21-PRACTITIONER-CALENDAR-CONFLICT-CLASSIFICATION.md`. #399 does not alter this rule.

**Provider credential security:** complete/do-not-redo. Dedicated Meta system user `Shiloh` owns the final production-only, least-privilege WhatsApp token; all former generic `Employee` tokens are revoked; only Render `WHATSAPP_TOKEN` changed. Preserve the two-scope permission boundary, production-only asset assignments and PR #387 redaction. Historic retained logs remain under Render retention and must not be destroyed by unsupported means. Earlier clean verification windows remain time-bounded; the repaired CRM onboarding incident does not reopen credential rotation.
