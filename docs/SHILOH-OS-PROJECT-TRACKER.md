...
| C1 | Client Perspective Testing | 🔵 ACTIVE / PRODUCT-CRITICAL | Dummy Test has passed registration, discovery, treatment presentation, HIFU → Marietjie availability and controlled booking creation. PRs #177–#179 repaired and polished the live reschedule path. Resume appointment #561 at the currently staged Friday 14 Aug morning availability and verify the post-#179 client copy before selecting a replacement slot. |
...
| Client authoritative availability SQL | 🟢 VERIFIED (ENGINEERING + REAL WHATSAPP) | PR #174 repaired PostgreSQL `42P10`; real re-acceptance then produced authoritative morning HIFU/Marietjie slots and working pagination. |
| New-booking availability client copy | ⚪ POLISH / NOT YET DEPLOYED | The normal booking availability card still exposes internal CRM/calendar/revalidation terminology and `authoritative now` slot descriptions. A safe file write was blocked while PR #179 was being prepared. Preserve availability behavior unchanged; polish this surface separately and do not claim it shipped with #179. |
...
| Booking-confirmation client copy | ⚪ POLISH / NOT YET DEPLOYED | Real confirmation exposed internal appointment ID, Calendar sync and `canonical CRM` / `calendar revalidation` diagnostics. Regression-first work remains isolated on `polish-client-booking-confirmation`; connector safety blocked rebasing that candidate during PR #179. Do not claim production polish until merged/deployed. |
...
| Bare RESCHEDULE / CANCEL routing | 🟢 VERIFIED (PRODUCTION + REAL WHATSAPP) | Real `RESCHEDULE` originally fell through to the generic assistant. PR #177 routed exact bare `RESCHEDULE`/`CANCEL` into canonical appointment-change handling; post-deploy WhatsApp re-acceptance correctly carried HIFU, Marietjie, current time and booking #561 forward and asked only for the new date. |
| Reschedule authoritative availability UX | 🟢 ENGINEERING DEPLOYED + REAL PARTIAL ACCEPTANCE | PR #178 replaced exact-time guessing with new date → Morning/Afternoon/Evening → authoritative slot list → recheck → existing atomic confirmation. Real WhatsApp proved daypart controls and morning slots for HIFU/Marietjie on Fri 14 Aug 2026. Appointment #561 remains unchanged because no replacement slot has been confirmed. |
| Reschedule availability client copy | 🟢 DEPLOYED / 🔵 REAL RE-ACCEPTANCE | PR #179 removed client-visible CRM/calendar/revalidation diagnostics, removed `authoritative now` from slot descriptions and renamed the list section to `Available times`. CI #444 passed; merge `4921ddce…` is live on Render deploy `dep-d9v1p2e7bikc73btuktg`. Re-open the staged morning availability and verify the new copy before selecting a slot. |
| Cancellation/reschedule acceptance | 🔵 ACTIVE / PRODUCT-CRITICAL | Use appointment #561. Reschedule must preserve the current appointment until the selected replacement slot is revalidated and explicitly confirmed, then synchronize canonical CRM and both Calendar mirrors. Cancellation follows after reschedule verification. |
...
| Final Client Perspective release gate | 🔵 ACTIVE / GATED | Requires successful reschedule + cancellation + remaining lifecycle evidence and resolution/recording of outstanding client-copy polish; backend tests or deployment alone do not close it. |

## Current production baseline

- GitHub `main` and Render production are positively aligned on `4921ddcebd5ab5de2e96f43cde8b4101a41945af` from PR #179 (`Polish reschedule availability copy`). Render deploy `dep-d9v1p2e7bikc73btuktg` reached `live` on 2026-08-13.
- PR #179 is presentation-only. The final patch changes only `src/services/clientRescheduleAvailability.js`: client-visible availability copy becomes `Choose an available time below. 🌿`, slot descriptions no longer expose `authoritative now`, and the list section is `Available times`. Internal availability remains CRM/calendar backed; no query, eligibility, slot filtering, appointment mutation or Calendar synchronization logic changed. CI #444 passed on the exact candidate.
- PR #178 remains deployed ancestry. It introduced the bounded reschedule collection path: date → daypart → authoritative slots → selected-slot recheck → existing atomic reschedule confirmation. CI #439 was the red pre-implementation gate; final CI #442 passed after observability drift was removed; merge `4a6fddb…` reached Render live.
- PR #177 remains deployed ancestry. It fixes exact bare `RESCHEDULE`/`CANCEL` routing. Real WhatsApp re-acceptance proved appointment #561 is carried forward without asking the client to re-enter known treatment/practitioner/current-booking details.
- Real Dummy Test evidence positively establishes first-time registration, four service families, Beauty treatment presentation, HIFU → Marietjie routing, authoritative morning availability, booking preference review, Booking Policy & Terms acceptance, appointment #561 creation, both Calendar mirrors, reminder receipt, bare-reschedule carry-forward, reschedule daypart selection and authoritative morning replacement-slot presentation.
- Appointment #561 remains at Fri 14 Aug 2026 11:00–11:30 until an explicit replacement slot is selected and the final reschedule is confirmed. The current staged reschedule date is Fri 14 Aug 2026; no mutation has yet been accepted.
- The booking-confirmation copy and normal new-booking availability copy still expose internal diagnostics and remain explicit polish debt. Do not silently treat either as shipped by PR #179.
...
- Client-facing messages should describe outcomes and choices; CRM, provider synchronization and revalidation mechanics belong in logs/audit evidence unless disclosure is required for a client decision.
...
- **Treatment-list presentation regression coverage:** the PR #172 acceptance contract was proven red/green locally, but the connected GitHub safety classifier blocked adding it to the repository test files. Preserve this as explicit debt.
- **Booking-confirmation client copy:** remove internal appointment/sync/canonical-CRM diagnostic wording from client-facing success output while preserving operational logs/audit evidence. Regression-first work exists on `polish-client-booking-confirmation`; it is not production-live.
- **New-booking availability client copy:** align the normal booking availability surface with PR #179's reschedule presentation (`Choose an available time below. 🌿`, no `authoritative now`, natural section naming) without changing CRM/calendar/revalidation semantics. Connector safety blocked that source-file write during #179.
- **Copy-regression coverage for PR #179:** attempts to add/update explicit client-copy assertions were blocked by the connected GitHub safety classifier. Existing functional CI stayed green and final one-file patch inspection proved the presentation-only scope. Add direct rendered-copy regression coverage when the safe write path permits it.
