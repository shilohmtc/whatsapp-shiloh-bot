...
New chat: read Master + Tracker + latest reconciliation, currently `docs/SHILOH-OS-RECONCILIATION-2026-08-19-BOOKING-UPDATE-PRODUCTION-ACTIVATION.md`, plus `docs/SHILOH-OS-ENGINEERING-GOVERNANCE.md`, on GitHub `main`; verify applicable production/provider state; give the four-part checkpoint; obtain explicit approval before the first new substantial controlled action. The prior stale-suppression reconciliation remains durable safety evidence at `docs/SHILOH-OS-RECONCILIATION-2026-08-19-BOOKING-UPDATE-STALE-SUPPRESSION.md`, the 19 August Meta approval reconciliation remains durable provider evidence at `docs/SHILOH-OS-RECONCILIATION-2026-08-19-META-BOOKING-UPDATE-APPROVAL.md`, and the Christel catalogue reconciliation remains durable evidence at `docs/SHILOH-OS-RECONCILIATION-2026-08-18-CHRISTEL-SERVICE-CATALOGUE-CORRECTION.md`; a newer current reconciliation does not erase any of them. After that approval, continue the approved workstream automatically through ordinary engineering/deploy/verification/housekeeping. Stop for material scope/risk expansion, contradictory authority, or an existing fail-closed human/provider/evidence gate.
...
Current accepted production application code is **PR #332 / `bd6b3963b5ba8a9518d49d9502936521a986a7bb`**, **Suppress stale booking-update retries**. The current governance baseline is **PR #317 / `f2a78bb33db212f759ac5bb72f1d832ca11cc104`**. Full regression CI run **#1064** passed with **695 passed / 0 failed**. PR #332's Render auto-deploy `dep-da2q3eks728c73b4jfug` reached LIVE and terminally suppressed #575 / 674 with `appointment_already_ended` before any send. Booking-update production activation later completed on docs-only main `29cf4ebc249b8b85d66a1616a26e35bd9e9739a0`: one Render environment merge update set the exact booking-update template name and `WHATSAPP_BOOKING_UPDATE_ENABLED=true`; API deploy `dep-da2qovs9v7es73cqlrr0` reached healthy LIVE at 15:16:16 SAST. PR #328 remains the accepted Christel catalogue correction lineage and PR #326 remains the accepted Meta integration lineage included in this application. Later documentation-only repository heads/deploys do not supersede the accepted application-code baseline unless runtime code changes are verified.

## At-a-glance

| ID | Workstream | State | Evidence / next action |
|---|---|---|---|
| DEPLOY-CONVERGENCE | GitHub ↔ Render production | 🟢 VERIFIED | Application #332 / `bd6b3963...` passed CI #1064 (695/0). Activation used the same runtime baseline on docs-only main `29cf4ebc...`; API deploy `dep-da2qovs9v7es73cqlrr0` reached healthy LIVE at 15:16:16 SAST after exact booking-update configuration/enablement. PR #328 catalogue and PR #326 Meta integration remain included. Governance baseline is #317 / `f2a78bb...`. |
...
| GOOGLE-CALENDAR-AUTH | Google Calendar OAuth/provider health | 🟢 VERIFIED HEALTHY | OAuth app In production; client/secret/refresh-token chain reconciled; activation restart again logged `Google Calendar provider health check passed`. |
...
**Authoritative now:** PR #326 remains the accepted Meta integration lineage included in the current application baseline. `shiloh_booking_update_v1` is **APPROVED**, `already_exists`, exact Utility / `en`, `duplicateCount=0`, quality `UNKNOWN`, and was not resubmitted. PR #332 is the accepted runtime safety baseline for terminal stale suppression. Production / DevOps has now completed exact template-name configuration and explicit booking-update enablement; activation deploy `dep-da2qovs9v7es73cqlrr0` reached healthy LIVE.
...
**Authoritative current state:** PR #332 / `bd6b3963b5ba8a9518d49d9502936521a986a7bb` remains the accepted regression-green production application baseline; CI #1064 passed 695/0. PR #328's Christel catalogue correction and PR #326 Meta integration remain included. `shiloh_booking_update_v1` is **APPROVED / existing / exact Utility `en` / duplicate-free / no resubmission / LIVE / ENABLED**. Activation deploy `dep-da2qovs9v7es73cqlrr0` reached healthy LIVE. Appointment #575 / audit event 674 is terminally suppressed for `appointment_already_ended`, `sent_at=null`, and is historical queue evidence only, never successful delivery evidence. Christel, Abigail and Marietjie each still finalize only their own appointments; JP finalizes none. Governance baseline #317 remains authoritative. Google Business Profile remains an external/provider gate with general Requests/min at 0.

**Highest-priority next item:** resume the already-active historical attendance review through explicit human truth. Booking-update customer delivery evidence remains passive/open until a genuine still-future appointment change occurs naturally; do not create work merely to close that evidence gate.

**Why next:** booking-update provider, safety and production activation work is complete. A delivery-evidence-only gate cannot justify a manufactured journey, while historical attendance remains an active human-truth workflow.

**Remaining gates:** successful booking-update customer delivery requires a natural future appointment change; #575 remains terminally suppressed historical evidence only. Control/business approval remains required before any Goldie-description publication. Other gates remain: Google Business Profile positive provider approval or usable Requests/min >0; historical attendance and #558 human truth; and other natural handset/lifecycle evidence.
