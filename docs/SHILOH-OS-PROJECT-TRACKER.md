...
**Next controlled step:** select **Browse treatments** from the registered-client list. Expected response is CRM-backed active client-bookable category discovery headed **Browse Shiloh services**. Do not select a treatment yet; stop for exact handset evidence.
...
**Highest-priority next item:** continue the controlled Juvan production client journey one handset step at a time. From the registered-client `Choose an option` list, select **Browse treatments**, then stop for the exact handset response before selecting any treatment.

**Why next:** #337 closed the greeting-routing defect with trace, regression, CI, deployment/provider-health verification and real handset proof. The remaining user goal is safe validation of the client menus and booking path without manufacturing or mutating an appointment. PR #340 changes governance/routing only and does not reopen or supersede that operational continuation.
