...
| C1 | Client Perspective Testing | 🔵 ACTIVE | C1.9 is code-level verified and C1.10 remains active. PR #164's synthetic privacy rollback simulator is live; continue non-destructive Calendar/retention/governance work without inventing staff or legal policy. |
...
| Client privacy/data minimization | 🔵 ACTIVE | Simulator sub-step is complete. Calendar payload audit confirms visible summary/description duplicate client/service/practitioner detail already present in private machine metadata; treatment visibility itself may be operationally necessary and must not be guessed away. Continue safe retention inventory/classification and provider/cross-border governance work while destructive execution remains disabled. |
...
- GitHub `main` and Render production were positively aligned on `e85d408b6ba0a7a1509530ee97e2f969f5133cb1` after PR #164 (`Add synthetic transaction rollback simulator`) reached `live` on 2026-08-13. Re-verify at the start of every new engineering session.
...
| **Client Perspective Testing / Client Testing Update** | 🟢 RECONCILED | Dedicated handoff plus master C1–C3 and this tracker preserve the current audit, fixes through PR #164, live-acceptance blockers and practitioner-information requirement. |
