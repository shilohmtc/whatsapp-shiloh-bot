...
|---|---|---|---|
| DEPLOY-CONVERGENCE | GitHub ↔ Render production | 🟢 VERIFIED | Application #328 / `78ece8d...` passed CI #1051 (688/0) and is LIVE as `dep-da2ba6f10e5c73cp6l60`; guarded DB startup passed, no post-deploy error/fatal logs were present, and `/health` returned application/database OK. PR #326 Meta integration is included; PR #330 catalogue reconciliation is preserved. Governance baseline is #317 / `f2a78bb...`. |
...
## Final Meta production reconciliation — complete

**Authoritative now:** the current production application baseline is PR #328 / `78ece8d5bcf38aaf5d01dbaaefacde253bdef6e3`; final CI #1051 passed and deploy `dep-da2ba6f10e5c73cp6l60` is LIVE and healthy with migration `062_christel_service_catalogue_correction.sql` applied successfully. PR #326 remains the accepted Meta integration lineage: merge `acefd0e3bc5a21c6e61c656ccfd8f185339c4783`, final head `9328a89b84bdeeebbee9ab1d3b74af809a30e017`, CI #1049 and original verified deploy `dep-da2b5ogu01pc73blgvn0`. That integration is included in PR #328; its production audit HTTP 200 with `report.ok=true` remains authoritative Meta evidence.

**Complete — do not redo:** the read-only inventory, exact-contract fail-closed enforcement, approval-resend evidence persistence, duplicate-safe post-acceptance follow-up handling, legacy evidence-endpoint retirement, and configuration/language/category/component/duplicate reconciliation are delivered. Eleven current configured operational identities are provider **APPROVED**, match exact name/language/category/components, have `duplicateCount=0`, and report `ready=true`. Tracker and durable Master reconciliation are complete. No application behaviour, environment variable, provider template, CRM record, appointment, attendance action, reminder, rating, birthday journey or WhatsApp message was changed or manufactured by this reconciliation.

**Remaining gates and owners:**

| Dependency | Gate | Owner | Control / observer |
|---|---|---|---|
| `shiloh_booking_update_v1` | Exact Utility/`en` contract exists with `duplicateCount=0`, but API status is **PENDING**, quality `UNKNOWN`, `configuredName=null`, and `ready=false`; await approval and intentional production configuration/enablement before a natural booking change. | **WhatsApp / Meta Integration** monitors provider approval; **Production / DevOps** owns configuration only after approval. | **Control & Reconciliation** tracks; **Booking & Admin UX** supplies only a genuine future journey. |
...
**Authoritative current state:** PR #328 is regression-green, merged and verified LIVE on `78ece8d...` as Render deploy `dep-da2ba6f10e5c73cp6l60`. Service #27 is inactive/unmapped with seven linked appointments preserved; #34/#65 remain active at 120/50; the reviewed canonical totals are 60/90/90. PR #326 Meta integration is included and its production audit returned HTTP 200 with `report.ok=true`; PR #330’s catalogue reconciliation remains preserved. Christel, Abigail and Marietjie each still finalize only their own appointments; JP finalizes none. PR #320/#322 menu presentation and JP's #318 Christel+Abigail booking entitlement remain unchanged. Governance baseline #317 remains authoritative. Google Business Profile remains an external/provider gate with general Requests/min at 0.

**Highest-priority next item:** Control & Reconciliation should consume the completed CRM checkpoint and continue the already-active historical attendance review only through safely routed explicit human truth. CRM & Identity has no remaining implementation action for the Christel correction.

**Why next:** the catalogue correction has completed implementation, 688/0 CI, merge, Render/database/public-catalogue verification and reconciliation without manufacturing an appointment. Historical attendance remains a separate human-truth workflow; Goldie descriptions remain a separate approval-gated content decision.

**Remaining gates:** no Booking & Admin UX or Production / DevOps dependency remains for the completed Christel correction. Control/business approval remains required before any Goldie-description publication. `shiloh_booking_update_v1` remains Meta PENDING with production configuration intentionally absent; approval-outcome, decline, cancellation, reschedule, rating-response and birthday evidence must arise only from genuine journeys. Other gates remain: Google Business Profile positive provider approval or usable Requests/min >0; historical attendance and #558 human truth; and natural handset evidence.
...
PR #326 / `acefd0e3bc5a21c6e61c656ccfd8f185339c4783` is merged, deployed and provider-verified. Final CI #1049 passed on `9328a89b84bdeeebbee9ab1d3b74af809a30e017`; original deploy `dep-da2b5ogu01pc73blgvn0` was verified LIVE and healthy; the read-only production audit returned HTTP 200 with `report.ok=true`. The authoritative per-template status and unchanged exact contract appendix are in `docs/META-TEMPLATE-READINESS-MATRIX.md`. Eleven configured operational identities are exact, APPROVED, duplicate-free and ready. Booking update remains provider/configuration/genuine-journey gated; the three legacy identities remain non-sendable with non-authoritative full contracts. API quality remains `UNKNOWN` separately from WhatsApp Manager **Active – Quality pending** evidence.

The implementation, CI, deploy, health, provider reconciliation, Tracker reconciliation and durable Master reconciliation are complete and must not be redone. Permanent provider lead-time/no-speculative-submission and no-manufactured-evidence governance remain in force.
