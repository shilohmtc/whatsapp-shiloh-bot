...
**Current application:** PR **#409 / `696a2c669a3de7b21f8119f0786c707974c30ffd`**, *Remove legacy Admin command-dump fallback*. The change is presentation-only: genuinely unrecognized authenticated Admin text no longer exposes the legacy raw command syntax and instead receives `I didn't recognise that admin request. Send *Menu* to open Shiloh Admin.` Existing guarded Admin handlers, authorization, `admin.whatsapp_unrecognized_command` audit, and active Admin booking/date routing remain unchanged. PR #399 remains the durable authority for the CRM normalized-phone ambiguity repair and migration 072; PR #395 remains the durable authority for practitioner Google Calendar conflict classification.

**CI:** PR #409 workflow run **32567694026** passed on Node **24.14.1**; the full non-mutating regression passed **868/868**, with 0 failed and 0 skipped. Four focused cases prove that raw legacy command syntax is no longer exposed, guarded Admin replies are unchanged, active mobile booking remains ahead of the generic Admin fallback, and startup preloads the presentation cleanup before `app.js` captures the Admin assistant export. PR #399's **860/860** ambiguity/onboarding regression and PR #395's focused **30/30** plus full **856/856** remain durable evidence for their own bounded scopes.

**Render application baseline:** PR #409 auto-deploy **`dep-da4njtlckfvc73cmnk0g`** reached **LIVE** on exact application merge `696a2c669a3de7b21f8119f0786c707974c30ffd`. Reconciliation PR #410 then merged as documentation-only commit `560099cdb55260ae045ffa6a2a3cb2cfdb51017b`; its auto-deploy **`dep-da4nrhgjo6nc73fee4sg`** reached **LIVE** at `2026-08-22T10:46:11.098874Z`. That final startup logged **Google Calendar provider health check passed**, verified migration 072 and the existing bounded runtime foundations, logged `Shiloh started`, and returned repeated `/health` HTTP 200 plus root HTTP 200. This newer clean health evidence supersedes the earlier transient Calendar permission warning seen during the #409 verification window. The documentation deploy does not supersede #409 application behavior.

**CRM onboarding ambiguity:** the 2026-08-22 PostgreSQL `42702` incident remains repaired and **🟢 VERIFIED LIVE** under PR #399. Root cause was migration 067's `client_contacts` trigger function declaring local `normalized_phone` and evaluating `WHERE d.normalized_phone = normalized_phone`; PostgreSQL treated the identifier as ambiguous before ordinary onboarding could complete. Applied migration 067 remains checksum-authoritative and was not edited; forward migration 072 repairs only the function. #409 does not alter that contract.

**Imported contact-book identity audit:** **🟢 VERIFIED READ-ONLY / COMPLETE; VERIFIED-CLIENT IMPLEMENTATION AUTHORIZED.** The approved Q1–Q11 pack executed against production on 2026-08-22 over TLSv1.3 inside an explicitly READ ONLY transaction that ended with `ROLLBACK`; no production mutation occurred. Snapshot: 794 active `goldie_import` clients; 776 phone-bearing; 553 phone-bearing with no appointment history; 241 with appointment history covering 530 appointment rows; derived 223 phone + history and 18 history without a current mobile/WhatsApp phone; 776 unique imported-origin phone groups and 0 groups resolving to >1 active canonical client; 3 current WhatsApp contacts; 4 `verified_at` proxy/anomaly records; 0 complete onboarding sessions; and no durable identity/onboarding/contact/claim/registration audit action for the cohort. `goldie_import`, imported labels/demographics, exact-phone uniqueness, profile completeness, history, contact type and `verified_at` alone are not verified identity authority. Control approved a forward explicit-verification evidence migration plus one centralized verified-client resolver consumed by CRM onboarding and Booking/Admin. No bulk remediation/trust backfill is authorized. Linda exact-phone remains blocked; no display-name lookup. CRM & Identity owns implementation; Booking & Admin UX is a mandatory shared-contract consumer.
...
|---|---|---|---|
| DEPLOY-CONVERGENCE | GitHub ↔ Render | 🟢 VERIFIED | Application authority is #409 / `696a2c669a3d...`; exact application deploy `dep-da4njtlckfvc73cmnk0g` reached LIVE. Reconciliation main `560099cdb552...` then deployed as `dep-da4nrhgjo6nc73fee4sg` and reached LIVE with Google Calendar health passed and repeated `/health` 200. Documentation-only convergence does not supersede #409 application behavior. |
| PRACTITIONER-CALENDAR-CONFLICT | Booking & Admin UX | 🟢 VERIFIED LIVE | #395 / `485ed97d8812...`; CI #1228 passed, focused 30/30, full 856/856. `dep-da4a75lckfvc738ghpmg` reached LIVE on exact merge. Unrelated practitioner Calendar events no longer block another assigned practitioner; shared/clinic-wide conflicts and all standing fail-closed guards remain blocking. #409 does not alter this rule. Do not redo. |
| CRM-ONBOARDING-NORMALIZED-PHONE-AMBIGUITY | CRM & Identity | 🟢 VERIFIED LIVE / COMPLETE | #399 / `26ace1027e10...`; migration 072 replaces only the migration-067 trigger function's ambiguous local identifier with `v_normalized_phone`. CI #1236 passed 860/860 plus four focused cases. `dep-da4me5qd0e5s73bobfm0` reached LIVE; migration 072 applied/checksum-verified, 065–068 remained checksum-valid, Juvan remained BOUND, repeated `/health` returned 200 and the bounded post-cutover window contained zero `normalized_phone` matches. #409 does not alter this repair. Do not redo. |
| CRM-IMPORTED-CONTACT-PROVENANCE-AUDIT | CRM & Identity + Production/DevOps observer | 🟢 VERIFIED READ-ONLY / COMPLETE | Q1–Q11 executed at the 2026-08-22 11:11:35 UTC snapshot over TLSv1.3 in an explicitly read-only transaction ending `ROLLBACK`. Current cohort: 794 active imported-origin clients; 553 imported-contact-only/no-history; 223 phone-bearing historical; 18 historical/no-current-phone; 776 unique imported phone groups; 0 multi-active phone conflicts; 0 complete onboarding sessions. Imported provenance/profile/history/`verified_at` alone are not verified identity authority. Linda exact-phone remains blocked/no display-name lookup. Do not rerun merely for reconciliation. |
| CRM-VERIFIED-CLIENT-AUTHORITY | CRM & Identity + Booking & Admin UX contract consumer | ⚪ READY / AUTHORIZED | HIGH-risk identity/privacy repair approved now: add forward explicit verification evidence, centralize one verified-client resolver, collect imported claimants' canonical data afresh without imported-label comparison or DOB/gender seeding, preserve client IDs/history/provenance and exact-phone duplicate protection, fail closed on historical/conflicting identity, and migrate Booking/Admin gates/commit to the same resolver. No bulk cleanup, trust backfill, or manual change to the four `verified_at` anomaly records under this approval. |
...
| ADMIN-UX-STANDARDIZATION | Booking & Admin UX | 🟢 VERIFIED LIVE | #371-#373 standardize Admin copy without changing WhatsApp-native typography or authority. `Shiloh Admin 🌿` + personalized welcome remain; landing prompt is `What would you like to manage today?`; `New booking`, `Manage booking`, and `Cancel new booking` are standardized; existing-appointment `Cancel booking` remains canonical. #409 extends only the unrecognized-input presentation boundary; the #371-#373 semantics remain authoritative. |
| ADMIN-LEGACY-FALLBACK-CLEANUP | Booking & Admin UX | 🟢 VERIFIED LIVE / COMPLETE | #409 / `696a2c669a3d...`; CI run 32567694026 passed 868/868; `dep-da4njtlckfvc73cmnk0g` reached LIVE on exact merge. Reconciliation #410 also reached LIVE as `dep-da4nrhgjo6nc73fee4sg` with clean provider/startup health. Unrecognized authenticated Admin text no longer exposes raw command syntax and receives compact Menu recovery only. Guarded commands, authorization, audit and active booking/date routing are unchanged. No business-data mutation. Do not redo. |
...
| ADMIN-BLOCK-TIME | Booking & Admin UX | 🟢 VERIFIED LIVE | #360 uses canonical `calendar_blocks`; Christel→Myself/Abigail, Abigail→self, Marietjie→self, JP/others→none; overlap/authority fail closed; no fake client appointment/message. |
...
| GOOGLE-CALENDAR-AUTH | Booking/Admin + Production | 🟢 VERIFIED HEALTHY | #302 fail-closed provider guard remains permanent and #395 remains the practitioner-conflict classification authority. Final reconciliation deploy `dep-da4nrhgjo6nc73fee4sg` logged `Google Calendar provider health check passed`; the earlier transient permission warning is superseded. Shared/clinic-wide and relevant assigned-practitioner conflicts remain blocking; unrelated practitioner events follow #395. |
...
- Admin and client cancellation paths now handle multi-staff bookings by locking every assigned practitioner and clearing all practitioner Calendar mirrors. Existing client self-service reschedule remains fail-closed for `staff_count != 1`, so Couples Massage reschedule is assisted until separately designed.
...
Durable evidence: `docs/SHILOH-OS-RECONCILIATION-2026-08-20-ADMIN-UX-STANDARDIZATION.md` and `docs/SHILOH-OS-RECONCILIATION-2026-08-22-ADMIN-LEGACY-FALLBACK-CLEANUP.md`.
...
- #409 removes only the legacy raw command-dump presentation for genuinely unrecognized authenticated Admin text. The replacement is `I didn't recognise that admin request. Send *Menu* to open Shiloh Admin.` Guarded replies, audit and active booking/date routing are unchanged.
- #409 CI passed 868/868 and exact Render deploy `dep-da4njtlckfvc73cmnk0g` reached LIVE on merge `696a2c669a3de7b21f8119f0786c707974c30ffd`; final reconciliation deploy `dep-da4nrhgjo6nc73fee4sg` then reached LIVE with Google Calendar health passed and repeated `/health` 200.
...
Booking confirmation v2 is live behind the exact centralized contract gate; v1 remains the explicit rollback. Genuine v2 handset delivery is not yet evidenced and may arise only from a natural booking. Booking update activation is complete but natural delivery evidence remains open. Google Calendar remains synchronized/fail-closed under #302/#395, and final reconciliation deploy `dep-da4nrhgjo6nc73fee4sg` logged the provider health check passed. Attendance stays own-practitioner only and #558 stays HOLD. Christel catalogue correction remains authoritative. Abigail is no longer eligible for service #31 Jaw Release; Christel remains the verified current mapping. Couples & Packages remains the client navigation owner; Couples Massage is now self-service under the exact #380 dual-practitioner contract. GBP, Ozow and privacy gates remain unchanged. Goldie description source identity is verified, while recovery and publication approvals remain fail-closed.

## Exact continuation

**Authoritative current state:** PR #409 / `696a2c669a3de7b21f8119f0786c707974c30ffd` is current accepted application code. Workflow run 32567694026 passed on Node 24.14.1; full non-mutating regression passed 868/868 with 0 failed/0 skipped and four focused fallback/routing/startup cases. Application deploy `dep-da4njtlckfvc73cmnk0g` reached LIVE on exact #409 merge. Reconciliation PR #410 then merged documentation-only main `560099cdb55260ae045ffa6a2a3cb2cfdb51017b`; deploy `dep-da4nrhgjo6nc73fee4sg` reached LIVE at `2026-08-22T10:46:11.098874Z`, logged Google Calendar provider health passed, verified migration 072 and standing runtime foundations, logged `Shiloh started`, and returned repeated `/health` 200 plus root 200. #409 changes only the unrecognized Admin presentation boundary. PR #399 remains durable authority for normalized-phone ambiguity repair/migration 072; PR #395 remains durable authority for practitioner Calendar conflict classification.

**Imported contact-book identity audit:** **🟢 VERIFIED READ-ONLY / COMPLETE; VERIFIED-CLIENT IMPLEMENTATION AUTHORIZED.** Current 2026-08-22 production evidence was obtained through the approved Q1–Q11 pack over TLSv1.3 in an explicitly read-only transaction ending `ROLLBACK`. `goldie_import`/contact-book provenance is not verified Shiloh identity; exact-phone uniqueness remains candidate-selection and duplicate/conflict protection only. The current snapshot establishes 553 imported-contact-only/no-history records, 223 phone-bearing historical imported clients, 18 historical imported clients without a current phone claim path, 0 complete onboarding sessions, and 4 `verified_at` proxy/anomaly records that must not be grandfathered. The approved next unit adds forward explicit verification evidence and one centralized verified-client resolver used by CRM onboarding and Booking/Admin, collects imported claimants' canonical registration data afresh without imported-label comparison or DOB/gender seeding, preserves existing client IDs/history/provenance, and fails closed where independent verification is insufficient. No bulk production remediation/trust backfill is authorized. Linda remains untraced because no exact-phone anchor was supplied; do not display-name match. CRM & Identity owns implementation; Booking & Admin UX is a required shared-contract consumer.

**Booking/Admin completed state:** #409 legacy Admin fallback cleanup is verified live and must not be redone: genuinely unrecognized authenticated Admin text no longer exposes internal command syntax, guarded Admin capabilities/authorization/audit remain unchanged, and active booking/date routing remains ahead of the generic fallback. #395 practitioner Calendar conflict classification remains verified live; #366 Primary/Backup approval remains verified live; #388 JP-only guarded Juvan booking cleanup remains verified live; #367 Manage booking guarded cancellation remains verified live with #380 multi-staff safety; #371-#373 presentation standardization remains verified live; #375/#376 remove service #31 from Abigail only; #378 established Couples & Packages navigation; #380 establishes atomic Couples Massage self-service. Current controlled Juvan identity remains phone-anchored/current-pointer based; do not name-match or permanently hard-code client 845.
...
**Admin presentation:** keep `Shiloh Admin 🌿`, personalized `Welcome back, <Admin> 👋`, and `What would you like to manage today?`; keep `Body Treatments`, `New booking`, `Manage booking`, and `Cancel new booking` semantics from #371-#373. Existing appointment cancellation remains `Cancel booking` through the canonical #367 flow. For genuinely unrecognized Admin text, keep #409's compact recovery `I didn't recognise that admin request. Send *Menu* to open Shiloh Admin.` and do not restore the legacy raw command dump. Active booking/date input must continue to route before this fallback.
...
**CRM onboarding ambiguity repair:** complete/do-not-redo. Migration 067 remains unchanged/checksum-authoritative; migration 072 removes the PL/pgSQL `normalized_phone` identifier collision while preserving controlled identity guards. PR #399 / `26ace1027e10...` remains the durable repair authority through CI #1236, 860/860 regression, exact Render deploy `dep-da4me5qd0e5s73bobfm0`, startup checksum evidence and bounded post-cutover error absence. #409 does not alter that repair.
