...
| Client privacy/data minimization | ⚪ READY | P-PRIV-1 through the non-destructive P-PRIV-4 gates are implemented. Next: synthetic transaction/rollback execution-plan simulator, then preserve Calendar minimization, historical-data retention classification, provider/subprocessor register, cross-border/POPIA governance and owner/legal-basis work; destructive execution remains disabled. |
