...
**Render application baseline:** PR #395 deploy **`dep-da4a75lckfvc738ghpmg`** reached **LIVE** on exact application merge `485ed97d8812fc291c71493dd1bb652b5da42f05`. Documentation-only PR #396 then auto-deployed as **`dep-da4abn9t0dsc73adlld0` / `9bd3251122475d9b2b36fffa2408b8e95d442bfc`** and was verified LIVE with repeated `/health` 200 responses before later documentation-only reconciliation. Documentation-only commits after #396 do not supersede #395 application authority; verify the exact current Render commit/deploy at continuation time rather than treating a documentation deploy ID as durable application authority.

**Newer production defect evidence:** current Render logs on 2026-08-22 contain three WhatsApp onboarding failures with PostgreSQL **42702**, `column reference "normalized_phone" is ambiguous`, in the `completeOnboarding()` → `processActiveSession()` path in `src/services/clientIdentityOnboarding.js`. This is newer production authority than earlier clean log windows. It does not invalidate #395 Calendar verification; CRM & Identity owns the bounded defect investigation/repair. Do not diagnose or mutate from Control by inference.
...
|---|---|---|---|
| DEPLOY-CONVERGENCE | GitHub ↔ Render | 🟢 VERIFIED | Application authority is #395 / `485ed97d8812...`. #396 / `9bd325112247...` and deploy `dep-da4abn9t0dsc73adlld0` are preserved as the verified pre-#397 documentation-only convergence point; later documentation-only merges do not supersede #395 application behavior. Verify exact current GitHub/Render convergence when continuation begins. Preserve newer CRM onboarding error evidence separately; health 200 does not erase application-path errors. |
...
**Authoritative current state:** PR #395 / `485ed97d8812fc291c71493dd1bb652b5da42f05` is current accepted application code. CI #1228 passed; focused tests passed 30/30 and full regression passed 856/856. Deploy `dep-da4a75lckfvc738ghpmg` reached LIVE on exact merge SHA. Documentation-only PR #396 / `9bd3251122475d9b2b36fffa2408b8e95d442bfc` passed CI #1230 and reached LIVE as `dep-da4abn9t0dsc73adlld0` before later documentation-only reconciliation. Subsequent documentation-only merges do not supersede #395 application authority; exact current GitHub/Render convergence must be re-verified at continuation time. PR #387 provider-log redaction, PR #385 Meta reconciliation, booking-confirmation v2 selection/evidence, PR #382 reschedule activation, PR #380 Couples Massage and all standing fail-closed invariants remain preserved.
