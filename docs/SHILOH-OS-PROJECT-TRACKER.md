...
Current accepted production application code is **PR #337 / `59119128ea0b288292622fd0b032058fcbd203ce`**, **Preserve universal welcome before greeting navigation**. The current governance baseline is **PR #317 / `f2a78bb33db212f759ac5bb72f1d832ca11cc104`**. Full regression CI run **#1074** passed with **701 passed / 0 failed**. PR #337's Render auto-deploy `dep-da2udf95efls73ba76ng` reached LIVE and healthy; repeated `/health` requests returned HTTP 200, Google Calendar provider health passed, and existing booking-update/cancellation/provider checks remained approved/existing without resubmission. A real Juvan handset `Hi` at about 19:31 SAST then received the universal welcome first and the registered-client four-row interactive list second; Render independently correlated the inbound text, outbound activity, successful four-row list send and HTTP 200 webhook completion. PR #335 remains the accepted authenticated SELECT-only diagnostic lineage, PR #332 remains the accepted stale booking-update suppression lineage included in this runtime, PR #328 remains the accepted Christel catalogue correction lineage, and PR #326 remains the accepted Meta integration lineage. Booking-update production remains LIVE / ENABLED under the previously reconciled #334 activation state.

## At-a-glance

| ID | Workstream | State | Evidence / next action |
|---|---|---|---|
| DEPLOY-CONVERGENCE | GitHub ↔ Render production | 🟢 VERIFIED | Application #337 / `59119128...` passed CI #1074 (701/0) and Render auto-deploy `dep-da2udf95efls73ba76ng` reached healthy LIVE. #337 repairs first-contact greeting routing only. PR #335 diagnostics, PR #332 stale-suppression, PR #328 catalogue, PR #326 Meta integration and #334 booking-update activation remain preserved. Governance baseline is #317 / `f2a78bb...`. |
...
| GOOGLE-CALENDAR-AUTH | Google Calendar OAuth/provider health | 🟢 VERIFIED HEALTHY | OAuth app In production; client/secret/refresh-token chain reconciled; #337 restart again logged `Google Calendar provider health check passed`. |
...
**Authoritative current state:** PR #337 / `59119128ea0b288292622fd0b032058fcbd203ce` is the accepted regression-green production application baseline; CI #1074 passed 701/0 and Render deploy `dep-da2udf95efls73ba76ng` is LIVE/healthy. Universal-welcome routing is repaired and handset-proven. PR #335 authenticated diagnostics, PR #332 stale-suppression, PR #328 catalogue and PR #326 Meta lineages remain included. `shiloh_booking_update_v1` remains **APPROVED / existing / exact Utility `en` / duplicate-free / no resubmission / LIVE / ENABLED**. Appointment #575 / audit event 674 remains terminally suppressed historical evidence only. Governance baseline #317 remains authoritative.
...
**Why next:** #337 closed the greeting-routing defect with trace, regression, CI, deployment/provider-health verification and real handset proof. The remaining user goal is safe validation of the client menus and booking path without manufacturing or mutating an appointment.
