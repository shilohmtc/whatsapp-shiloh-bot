...
### 2026-08-14 verified reconciliation — supersedes older C1/C3/baseline wording below where it conflicts

- GitHub `main` and Render production reached **`aa79cfa0064f2ec0ca0801b42a07a82c37945345`** after PR #189 (`Include client mobile in booking calendar descriptions`) was squash-merged; Render deploy **`dep-d9va4s49v7es7399tg4g`** reached `live`. PR #189 was self-test-first: regression-only commit `5457cbc53bb9e446dc2cb828a16c43a6c49496bf` failed CI #477 before implementation and final head `b1d8553fcda3f7cf7e62b21f702301b6c7d214f8` passed CI #481. Future Shiloh-created shared and practitioner Calendar descriptions include the client mobile while titles remain unchanged; later Calendar update/reschedule patches preserve the mobile metadata. No existing appointment was backfilled. Real next-booking Calendar presentation acceptance remains outstanding and must not be forced by creating a booking solely for this check.
- PR #188 (`Add permission-gated read-only client details`) is production-live. It preserves the ordinary masked client lookup and adds an explicit `Find client details #<CRM_ID>` path that reuses `client:lookup`, revalidates Admin scope and performs SELECT-only detail retrieval. Regression-only CI #471 failed before implementation; final CI #475 passed. Real Admin acceptance against CRM #836 returned full canonical contact detail and explicitly changed no client identity/contact records.
- The real Dummy Test appointment-management journey for **#561** is now complete through cancellation. A proven defect showed bare `CANCEL` being swallowed by an active reschedule-availability state; PR #187 fixed command precedence self-test-first. Post-deploy real WhatsApp acceptance then showed `CANCEL` resolving #561 and presenting explicit confirmation; the tester sent `YES`; Shiloh confirmed cancellation; independent searches showed #561 absent from both `Shiloh — Bookings` and `Shiloh — Marietjie`. No replacement booking was created. Do not reset/recreate #561 merely to repeat accepted booking/reschedule/cancellation steps.
- Zane Maree CRM #836 was used as a read-only Admin-detail acceptance target. The independently reconciled current appointment is **#562 — Full Body Swedish — Abigail — 14 Aug 2026 15:00–16:45**, mirrored on `Shiloh — Bookings` and `Shiloh — Abigail`; no CRM or appointment mutation was performed during that lookup/reconciliation.
- The Product-Critical Gate remains **real Client Perspective acceptance**, but the next actionable evidence is now registered-client return / remaining client-facing copy / lifecycle-template acceptance rather than cancellation. The older 2026-08-13 C1/C3 bullets and the older recommended execution order below are historical where they still say cancellation is pending or privacy work is the immediate execution priority.
...
- PR #165 completes the safe read-only Goldie historical-retention inventory/classification sub-step. Self-test-first evidence remains explicit: branch CI #385 failed before the inventory implementation existed; corrected branch CI #387 passed; a fresh production-candidate branch was then created without the temporary branch-only CI trigger, and PR CI #388 passed on that exact two-file candidate. `goldieRetentionInventory` performs only sanitized count queries and boolean configuration-presence checks, never decodes or returns the historical future-import environment payload, never selects raw payload values, fails closed if the historical tables cannot be read, and always returns `executionReady=false` / `destructiveActionAllowed=false`. It separates raw Goldie client staging/source copies (policy decision required), reconciliation provenance (retain pending legal basis), and non-personal historical catalogue/import provenance rather than treating all migration artifacts as one retention class. It does **not** delete, de-identify or establish final retention periods/legal authority.
...
- Privacy/data-minimization hardening already present in the deployed ancestry includes P-PRIV-1 through the currently implemented P-PRIV-4 gates plus PR #164's synthetic rollback simulator and PR #165's read-only Goldie retention inventory: fail-closed AI profile preference allowlisting, bounded local OpenAI conversation-state retention, short-lived temporary onboarding/walk-in staging retention, non-destructive client privacy inventory/retention planning, privacy-request identity-verification/owner-authorization workflow state, pure in-memory execution-plan rehearsal, and sanitized historical-retention classification. Destructive privacy execution remains intentionally disabled.
...
- 14 Aug evidence: PR #188 added a separate permission-gated, scope-revalidated read-only client detail view while preserving masked ordinary search. Real Admin acceptance against CRM #836 passed. Do not weaken ordinary search masking now that authorized full detail has an explicit safe path.
...
The numbered 2026-08-13 status list below is retained as historical execution detail. **The 2026-08-14 reconciliation at the top of this Master supersedes it where it still says controlled booking/cancellation is pending.** Current truth: real Dummy Test #561 booking, reschedule and cancellation are accepted; PRs #187–#189 are production-live; registered-return/client-copy/lifecycle-template acceptance remains active.
...
10. 🔵 **Client privacy and data-minimization acceptance** — materially advanced but not complete. Evidence and rules reconciled from the full Hostname Change Implications/privacy source chat:
   ...
   - ✅ **Historical Goldie retention inventory/classification foundation:** PR #165 adds `goldieRetentionInventory` and dedicated regressions. The test-first sequence preserved a failing pre-implementation branch gate (#385), a green corrected branch gate (#387), and a clean two-file PR candidate with final CI #388 green. The inventory performs read-only counts only; returns no Goldie names, phones, emails, addresses, notes or raw payload values; reports the future-import environment payload only as configured/not-configured and never decodes it; fails closed if historical tables are unavailable; and always returns `executionReady=false` / `destructiveActionAllowed=false`. The classification now explicitly separates raw client staging and duplicated source payloads (`policy_decision_required`) from reconciliation provenance (`retain_pending_legal_basis`) and non-personal import/service-history provenance. This closes the engineering inventory/classification sub-step but **not** the final legal/owner retention decision or any destructive execution.
   ...
   - Remaining immediate P-PRIV-4 work after PR #165: keep real destructive/de-identification execution disabled; establish approved retention/legal basis and owner-facing approval handling before any real executor is considered; continue provider/subprocessor/cross-border governance; and resolve Calendar minimization only with authoritative staff diary requirements rather than inference. Do not mark this item complete until those consequences are evidenced or explicitly superseded.
...
- **14 Aug evidence:** the controlled Dummy Test journey positively proved registration, booking #561, both Calendar mirrors, reschedule, and explicit cancellation with both mirrors removed. The direct Render SQL connector still cannot provide independent CRM-row reads, while guarded application surfaces are now preferred where available. Keep C3 open only for genuinely unaccepted retry/registered-return/lifecycle consequences; do not recreate #561 to repeat accepted management steps.
...
**2026-08-14 override:** continue the Product-Critical real Client Perspective gate from the post-cancellation state. Do not return to privacy work merely because the older numbered order below said it was first; privacy safeguards remain mandatory but are not the immediate execution priority unless they reveal a safety blocker.

1. 🔵 Continue Client Perspective Testing at C1 item 10 (Client privacy and data-minimization acceptance). PR #164's synthetic transaction/rollback simulator and PR #165's read-only Goldie retention inventory/classification are complete and production-live; destructive execution remains disabled.
2. ⬜ Continue only non-destructive C1.10 work that does not require invented policy: provider/subprocessor/cross-border data-flow governance and Calendar minimization. Calendar treatment/service visibility remains decision-gated on authoritative staff workflow requirements; do not silently remove it.
3. ⬜ Establish approved Goldie/general retention periods/legal basis and owner-facing approval handling before considering any real destructive/de-identification executor. No current engineering evidence authorizes that executor.
