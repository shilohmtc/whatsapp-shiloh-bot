...
The production Shiloh_MTC WhatsApp Manager evidence below was captured on 18 August 2026, with fresh booking-update provider/activation evidence on 19 August 2026 and one controlled `shiloh_booking_confirmation_v2` Meta submission plus immediate read-only provider reconciliation on 20 August 2026. Provider approval is not handset delivery evidence and provider acceptance is not production activation. The checker must compare the complete provider contract and remain read-only except for an explicitly authorized controlled submission; no template is edited or sent merely for evidence.
...
| 11 | `shiloh_booking_confirmation_v2` | Utility / `en` | API **PENDING** after one controlled 20 Aug submission; immediate read-back exact; quality not established by this read-back | Exact provider name/language/category/components verified; `duplicateCount=0`; centralized inventory defines it but `sendable=false`; current production selector remains v1; one-shot provisioning flag returned to `false`; `ready=false` | **BLOCKED / NOT ACTIVE.** Await provider **APPROVED**, then re-verify exact contract + duplicate-free state before any separately approved code/config activation. Do not resubmit, switch production, or manufacture a booking for evidence. |
...
### `shiloh_booking_confirmation_v2` — Utility / `en` — provider PENDING / production inactive
- **HEADER:** TEXT `Appointment confirmed`.
- **BODY (exact):**
  ```text
...
- **Activation:** `sendable=false`, current production selector remains v1, provider status PENDING. Do not activate or resubmit.
