...
function createStaffPasskeyAuthRouter({
  ...
  function noStore(res) {
    ...
    res.setHeader('Pragma', 'no-cache'); res.setHeader('Referrer-Policy', 'no-referrer');
  }
  function secure(res) {
    noStore(res); res.setHeader('X-Content-Type-Options', 'nosniff'); res.setHeader('X-Frame-Options', 'DENY');
  ...
  router.post('/authentication/options', sameOrigin, async (req, res, next) => {
    try { const result = await passkeyService.beginAuthentication({ requestFingerprintHash: requestFingerprintHash(req) }); if (!result.ok) return error(res, result); noStore(res); return res.status(200).json({ options: result.options, expiresAt: result.expiresAt }); }
    catch (e) { return next(e); }
  });
  router.post('/authentication/finish', sameOrigin, async (req, res, next) => {
    try { const result = await passkeyService.finishAuthentication({ response: req.body?.response, requestFingerprintHash: requestFingerprintHash(req) }); if (!result.ok) return error(res, result); return sendSession(res, result); }
    catch (e) { return next(e); }
  });
  router.get('/manage', requireSession, async (req, res, next) => {
    try { const result = await passkeyService.listCredentials({ session: req.staffBrowserSession }); if (!result.ok) return error(res, result); secure(res); return res.status(200).type('html').send(managePage()); }
    catch (e) { return next(e); }
  });
  router.get('/manage.js', requireSession, async (req, res, next) => {
    try { const result = await passkeyService.listCredentials({ session: req.staffBrowserSession }); if (!result.ok) return error(res, result); secure(res); return res.status(200).type('application/javascript').send(manageScript()); }
    catch (e) { return next(e); }
  });
  router.get('/', requireSession, async (req, res, next) => {
    try { const result = await passkeyService.listCredentials({ session: req.staffBrowserSession }); if (!result.ok) return error(res, result); noStore(res); return res.status(200).json({ credentials: result.credentials }); }
    catch (e) { return next(e); }
  });
  router.post('/registration/options', sameOrigin, requireSession, requireCsrf, async (req, res, next) => {
    try { const result = await passkeyService.beginRegistration({ session: req.staffBrowserSession, requestFingerprintHash: requestFingerprintHash(req) }); if (!result.ok) return error(res, result); noStore(res); return res.status(200).json({ options: result.options, expiresAt: result.expiresAt }); }
    catch (e) { return next(e); }
  });
  router.post('/registration/finish', sameOrigin, requireSession, requireCsrf, async (req, res, next) => {
    try { const result = await passkeyService.finishRegistration({ session: req.staffBrowserSession, response: req.body?.response, requestFingerprintHash: requestFingerprintHash(req) }); if (!result.ok) return error(res, result); noStore(res); return res.status(201).json({ ok: true, credentialId: result.credentialId }); }
    catch (e) { return next(e); }
  });
  router.post('/:credentialId/revoke', sameOrigin, requireSession, requireCsrf, async (req, res, next) => {
    try { const result = await passkeyService.revokeCredential({ session: req.staffBrowserSession, credentialId: req.params.credentialId, requestFingerprintHash: requestFingerprintHash(req) }); if (!result.ok) return error(res, result); noStore(res); return res.status(204).send(); }
    catch (e) { return next(e); }
  });
  return router;
}
