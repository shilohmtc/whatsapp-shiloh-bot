    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1 FROM appointment_staff ast_scope
         WHERE ast_scope.appointment_id=${alias}.id AND ast_scope.staff_id=$2
      )
      AND EXISTS (
        SELECT 1
...
function pendingListInteractive(data) {
  ...
  if (data.hasNext) {
    rows.push({
      id: `finalize_appts_page_${data.page + 1}`,
      title: 'More appointments',
      description: 'Show older appointments awaiting final status',
    });
  }
  ...
  return {
    ...
    body: data.rows.length
      ? '*Finalize past appointments*\nChoose one past appointment that still needs an explicit final attendance status. Nothing changes until you choose Completed or No-show.'
      : '*Finalize past appointments*\nThere are no past appointments awaiting final status in your authorized scope.',
    buttonText: 'Past appointments',
    rows,
    sectionTitle: 'Awaiting final status',
  };
}
...
function decisionInteractive(appointment) {
  const details = [
    `*Finalize appointment #${appointment.id}*`,
    `👤 ${appointment.client_name}`,
    `✨ ${appointment.services || 'Service not recorded'}`,
    `💆 ${appointment.staff || 'Practitioner not recorded'}`,
...
async function finalizeAppointment(admin, appointmentId, targetStatus) {
  ...
  try {
    ...
    if (!appointment) {
      ...
      return { status: 'stale_or_forbidden' };
    }
    ...
    await db.query(
      ...
      [
        ...
        appointment.status,
        targetStatus,
        `admin:${admin.id}:${admin.display_name}`,
        'Explicit WhatsApp admin past-appointment finalization',
      ]
    );
    await db.query(
    ...
    await db.query(
      `INSERT INTO crm_audit_events
       ...
       VALUES ($1,'admin.appointment_finalized','appointment',$2,$3::jsonb)`,
      [
        admin.id,
...
async function processAdminAppointmentFinalizationMessage(sender, text) {
  ...
  if (!has(admin, 'appointment:view') || !has(admin, 'booking:update')) {
    return { handled: true, admin, reply: 'Your admin account does not currently have permission to finalize appointment status.' };
  }

  if (isEntry || pageMatch || isBack) {
    const page = pageMatch ? Number(pageMatch[1]) : 1;
    const data = await pendingPastAppointments(admin, page);
    return { handled: true, admin, interactive: pendingListInteractive(data) };
  }

  if (selectionMatch) {
    const appointment = await loadAuthorizedPendingAppointment(admin, Number(selectionMatch[1]));
    if (!appointment) {
      return { handled: true, admin, reply: 'That appointment is no longer awaiting final status in your authorized scope.' };
    }
    return { handled: true, admin, interactive: decisionInteractive(appointment) };
  }
  ...
  if (result.status !== 'updated') {
    return { handled: true, admin, reply: 'That appointment changed or is outside your authorized scope, so no status update was made.' };
  }
  const label = targetStatus === 'completed' ? 'Completed' : 'No-show';
  return {
    handled: true,
    admin,
    reply: `✅ Appointment #${appointmentId} marked *${label}*. Reporting integrity will now use this explicit canonical status.`,
  };
}
...
module.exports = {
  FINAL_STATUSES,
  PAGE_SIZE,
  pendingPastAppointments,
  pendingListInteractive,
  decisionInteractive,
  finalizeAppointment,
