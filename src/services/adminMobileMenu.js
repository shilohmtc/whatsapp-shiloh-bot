...
function menu(admin){
  ...
  if(has(admin,'schedule:manage')){lines.push('','*More*',`${n++}️⃣ Staff schedule`);}
...
function scheduleMenu(){return ['*Staff schedule*','','1️⃣ View working hours','2️⃣ Change regular hours','3️⃣ Add leave / special hours','4️⃣ Remove an exception','','Examples are shown only after you choose an option.','Reply MENU to return.'].join('\n');}
...
async function processAdminMobileMenuMessage(sender,text){
  ...
  if(v==='7'||v==='staff schedule'||v==='schedule'){if(!has(admin,'schedule:manage'))return {handled:false};await audit(admin.id,'admin.mobile_menu_selected',{option:'schedule'});return {handled:true,admin,reply:scheduleMenu()};}
  const sm=v.match(/^(?:schedule\s+)?([1-4])$/);if(sm&&has(admin,'schedule:manage'))return {handled:true,admin,reply:scheduleGuide(sm[1])};
