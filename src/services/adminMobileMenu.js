...
function isBusinessWide(admin){return ['owner','business_admin'].includes(admin?.business_role)||admin?.calendar_scope==='all_business';}
...
function getMenuOptions(admin){
  ...
  if(has(admin,'appointment:view')){
    options.push({key:'today',label:isBusinessWide(admin)?"Today's clients":"My clients today",section:'Appointments'});
    options.push({key:'tomorrow',label:isBusinessWide(admin)?"Tomorrow's clients":"My clients tomorrow",section:'Appointments'});
  ...
  if(has(admin,'booking:update')&&has(admin,'appointment:view')) options.push({key:'manage_booking',label:'Manage a booking',section:'Appointments'});
  if(has(admin,'client:lookup')) options.push({key:'client',label:isBusinessWide(admin)?'Find a client':'Find my client',section:'Clients'});
  if(has(admin,'walkin:create')) options.push({key:'walkin',label:'Add a walk-in',section:'Clients'});
  if(has(admin,'staff:services:view')) options.push({key:'staff_services',label:isBusinessWide(admin)?'Staff services':'My services',section:'Services'});
  if(has(admin,'service:pricing')) options.push({key:'pricing',label:isTenant(admin)?'My services & pricing':'Services & pricing',section:'Services'});
  if(has(admin,'schedule:manage')) options.push({key:'schedule',label:isTenant(admin)?'My schedule':'Schedule management',section:'Schedule'});
  options.push({key:'help',label:'Help',section:'More'});
  return options.map((o,i)=>({...o,number:i+1}));
}
function menu(admin){
  const roleLine=isTenant(admin)?'Tenant business access — your clients, services and diary only.':isEmployeePractitioner(admin)?'Practitioner access — your diary and assigned client work only.':'';
  const options=getMenuOptions(admin);const lines=['*Shiloh Admin 🌿*',`Welcome back, ${admin.display_name} 👋`];if(roleLine)lines.push(roleLine);lines.push('','What would you like to do?');let section=null;
...
function clientGuide(admin){return [isBusinessWide(admin)?'*Find a client*':'*Find my client*','',isBusinessWide(admin)?'Send the client name or mobile number after “Find client”.':'Search is limited to clients in your authorized practitioner/service scope.','','Example:','Find client Chenique'].join('\n');}
function scheduleMenu(admin){if(isTenant(admin))return ['*My schedule*','','1️⃣ My regular hours','2️⃣ My leave / special availability','','0️⃣ Back','','Reply with a number or option name.'].join('\n');return ['*Schedule management*','','1️⃣ Staff hours','2️⃣ Leave / special availability','3️⃣ Freelancer availability','4️⃣ Holiday hours','','0️⃣ Back','','Reply with a number or option name.'].join('\n');}
...
async function processAdminMobileMenuMessage(sender,text){
  ...
  if(selected){
    await audit(admin.id,'admin.mobile_menu_selected',{option:selected.key,businessRole:admin.business_role});
    if(selected.key==='today'||selected.key==='tomorrow')return {handled:false};
    ...
    if(selected.key==='client')return {handled:true,admin,reply:clientGuide(admin)};
    ...
    if(selected.key==='schedule'){moreSessions.set(k,{step:'menu'});return {handled:true,admin,reply:scheduleMenu(admin)};}
    if(selected.key==='help')return {handled:false};
  }
  if(v==='today'||v==="today's clients"||v==='todays clients'||v==='my clients today'||v==='tomorrow'||v==="tomorrow's clients"||v==='tomorrows clients'||v==='my clients tomorrow')return {handled:false};
  ...
  if(v==='find a client'||v==='find my client'){if(!has(admin,'client:lookup'))return {handled:false};return {handled:true,admin,reply:clientGuide(admin)};}
