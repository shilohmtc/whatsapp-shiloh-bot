...
const { getWorkingHours, addScheduleException, formatWorkingHours } = require('./staffScheduleService');
...
async function processAdminOwnScheduleMessage(sender,text){
 ...
 if(['my schedule','my regular hours','staff hours','regular staff hours'].includes(v)){
  ...
  return{handled:true,admin,reply:[`*${admin.display_name} — My schedule*`,'',formatWorkingHours(data),'','Regular weekly hours come from clinic operating hours.','','For dated leave/special availability, send *My leave*.'].join('\n')};
 }
 ...
 if(m)return{handled:true,admin,reply:'Regular practitioners use canonical clinic operating hours. Use *My leave* for dated leave or unavailable time.'};
