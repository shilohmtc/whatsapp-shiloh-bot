...
async function prepareHistoricalAdminBooking({ adminId, clientId, staffId, serviceId, date, time }) {
  ...
  await pool.query(
    `INSERT INTO admin_booking_sessions (admin_id,client_id,staff_id,service_id,location_id,starts_at,ends_at,state,updated_at)
     VALUES ($1,$2,$3,$4,$5,$6,$7,'confirm',NOW())
     ON CONFLICT (admin_id) DO UPDATE SET client_id=EXCLUDED.client_id,staff_id=EXCLUDED.staff_id,service_id=EXCLUDED.service_id,
       location_id=EXCLUDED.location_id,starts_at=EXCLUDED.starts_at,ends_at=EXCLUDED.ends_at,state='confirm',updated_at=NOW()`,
  ...
  return { status: 'pending_confirmation', reply: [
  ...
  try {
    ...
    const sessionResult = await db.query(
      `SELECT abs.admin_id,abs.client_id,abs.staff_id,abs.service_id,abs.location_id,abs.starts_at,abs.ends_at,abs.state,
              ...
              (SELECT COALESCE(NULLIF(cc.value,''),cc.normalized_value) FROM client_contacts cc WHERE cc.client_id=c.id
                AND LOWER(cc.contact_type) IN ('whatsapp','mobile','phone','telephone') ORDER BY cc.is_primary DESC,cc.verified_at DESC NULLS LAST,cc.id LIMIT 1) client_mobile,
         ...
         FROM admin_booking_sessions abs JOIN clients c ON c.id=abs.client_id JOIN staff st ON st.id=abs.staff_id
         JOIN services s ON s.id=abs.service_id JOIN locations l ON l.id=abs.location_id
        WHERE abs.admin_id=$1 FOR UPDATE OF abs`, [admin.id]);
