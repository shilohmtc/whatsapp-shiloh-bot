...
async function ensureTable() {
  ...
  await pool.query(`
    ...
    ON appointment_lifecycle (status, appointment_at)
  `);
...
async function createAppointment({ phone, service, appointmentAt, therapist, source = "admin" }) {
  ...
  const result = await pool.query(
    `INSERT INTO appointment_lifecycle
       (phone, service_text, appointment_at, therapist_text, source)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING *`,
...
async function listAppointments(limit = 100) {
  ...
  const result = await pool.query(
    `SELECT * FROM appointment_lifecycle
...
async function claimDueReminder() {
  await ensureTable();
  const result = await pool.query(
    `UPDATE appointment_lifecycle a
    ...
    for (let i = 0; i < 20; i += 1) {
      ...
      try {
        await sendWhatsAppTemplate(
          appointment.phone,
          reminderTemplate,
          [appointment.service_text, formatAppointmentDate(appointment.appointment_at)],
          LANGUAGE_CODE
        );
      } catch (error) {
