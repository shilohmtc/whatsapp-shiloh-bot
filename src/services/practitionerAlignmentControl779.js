...
function assertSourcePractitioner(staff, access) {
  ...
  if (staff.status !== 'active' || staff.resource_type !== 'practitioner') {
    throw new Error('#779 source is not an active canonical practitioner');
  }
  if (access.active !== true || Number(access.staff_id) !== Number(staff.id)) {
    throw new Error('#779 source access is not actively linked to source Staff');
  }
  if (access.role !== 'practitioner' || access.business_role !== 'employee_practitioner') {
    throw new Error('#779 source access is not the normal employee practitioner preset');
  }
  if (access.calendar_scope !== 'own_appointments' || access.service_scope !== 'own_services') {
...
async function loadNamedRows(queryable, table, name, { lock = false } = {}) {
  ...
  const result = await queryable.query(
    `SELECT * FROM ${table} WHERE LOWER(TRIM(display_name))=LOWER(TRIM($1)) ORDER BY id LIMIT 3${suffix}`,
    [name]
  );
  return result.rows || [];
}
...
async function loadSnapshot(queryable, sourceName, targetName, { lock = false } = {}) {
  ...
  if (targetStaff && Number(targetAccess.staff_id || 0) !== 0 && Number(targetAccess.staff_id) !== Number(targetStaff.id)) {
    throw new Error('#779 target access is linked to a different Staff row');
  }
  return {
    sourceStaff,
    sourceAccess,
    sourceServices: await loadServices(queryable, sourceStaff.id),
    targetStaff,
    targetAccess,
    targetServices: targetStaff ? await loadServices(queryable, targetStaff.id) : [],
  };
}
...
function sanitizedSnapshot(snapshot) {
  return {
    source: {
      staff: sanitizeStaff(snapshot.sourceStaff),
      access: sanitizeAccess(snapshot.sourceAccess),
      services: sanitizeServices(snapshot.sourceServices),
    },
    target: {
      staff: sanitizeStaff(snapshot.targetStaff),
      access: sanitizeAccess(snapshot.targetAccess),
      services: sanitizeServices(snapshot.targetServices),
    },
  };
}
...
function alreadyAligned(snapshot) {
  const s = snapshot.sourceStaff;
  const t = snapshot.targetStaff;
  const sa = snapshot.sourceAccess;
  const ta = snapshot.targetAccess;
  if (!t) return false;
  return t.resource_type === s.resource_type
    && t.status === s.status
    && t.scheduling_type === s.scheduling_type
    && t.client_bookable === s.client_bookable
    && t.business_role === s.business_role
    && t.calendar_scope === s.calendar_scope
    && Number(ta.staff_id) === Number(t.id)
    && ta.active === true
    && ta.role === sa.role
    && ta.business_role === sa.business_role
    && ta.calendar_scope === sa.calendar_scope
    && ta.service_scope === sa.service_scope
...
async function applyAlignment({ db = pool, sourceName, targetName } = {}) {
  ...
  const source = normalizeName(sourceName, 'source');
  const target = normalizeName(targetName, 'target');
  ...
  try {
    ...
    if (alreadyAligned(before)) {
      await client.query('COMMIT');
      return { status: 'unchanged', before: sanitizedSnapshot(before), after: sanitizedSnapshot(before) };
    }
    ...
    if (!targetStaff) {
      const inserted = await client.query(
        `INSERT INTO staff(display_name,resource_type,status,scheduling_type,client_bookable,business_role,calendar_scope,source_name)
         ...
         RETURNING *`,
        [
          before.targetAccess.display_name,
          before.sourceStaff.resource_type,
          before.sourceStaff.status,
          before.sourceStaff.scheduling_type,
          before.sourceStaff.client_bookable,
          before.sourceStaff.business_role,
          before.sourceStaff.calendar_scope,
        ]
      );
      ...
      const updated = await client.query(
        `UPDATE staff
            SET resource_type=$2,status=$3,scheduling_type=$4,client_bookable=$5,business_role=$6,calendar_scope=$7,updated_at=NOW()
          WHERE id=$1
          RETURNING *`,
        [
          targetStaff.id,
    ...
    await client.query(
      `UPDATE staff_admin_accounts
          SET staff_id=$2,role=$3,active=TRUE,permissions=$4::jsonb,business_role=$5,calendar_scope=$6,service_scope=$7,updated_at=NOW()
        WHERE id=$1`,
      [
        before.targetAccess.id,
        targetStaff.id,
        before.sourceAccess.role,
        JSON.stringify(before.sourceAccess.permissions || {}),
        before.sourceAccess.business_role,
        before.sourceAccess.calendar_scope,
        before.sourceAccess.service_scope,
      ]
    );

    await client.query(
      `DELETE FROM staff_services target
        WHERE target.staff_id=$1
          AND NOT EXISTS (
            SELECT 1 FROM staff_services source
             WHERE source.staff_id=$2 AND source.service_id=target.service_id
          )`,
      [targetStaff.id, before.sourceStaff.id]
    );
    await client.query(
      `INSERT INTO staff_services(staff_id,service_id)
       SELECT $1, source.service_id
         FROM staff_services source
        WHERE source.staff_id=$2
          AND NOT EXISTS (
            SELECT 1 FROM staff_services target
             WHERE target.staff_id=$1 AND target.service_id=source.service_id
          )`,
      [targetStaff.id, before.sourceStaff.id]
    );

    const after = await loadSnapshot(client, source, target, { lock: false });
    ...
    await client.query(
      `INSERT INTO crm_audit_events(actor_admin_id,action,entity_type,entity_id,metadata)
       VALUES(NULL,'control.practitioner_alignment_779','staff',$1,$2::jsonb)`,
      [targetStaff.id, JSON.stringify({
        issue: 779,
        sourceStaffId: Number(before.sourceStaff.id),
        targetStaffId: Number(targetStaff.id),
        sourceAdminId: Number(before.sourceAccess.id),
        targetAdminId: Number(before.targetAccess.id),
        identityMaterialCopied: false,
        authenticationMaterialChanged: false,
        providerSend: false,
        teamRoutingAuthorityChanged: false,
        before: sanitizedSnapshot(before).target,
        after: sanitizedSnapshot(after).target,
      })]
    );
...
async function runConfiguredAlignment({ db = pool, env = process.env, log = logger } = {}) {
  ...
  const sourceName = normalizeName(env?.[SOURCE_KEY], 'source');
  const targetName = normalizeName(env?.[TARGET_KEY], 'target');
  if (mode === 'audit') {
    ...
    log.info(result, '#779 sanitized practitioner alignment audit');
    return result;
  }
  const result = await applyAlignment({ db, sourceName, targetName });
  log.info(result, '#779 practitioner alignment applied and verified');
  return result;
}

if (require.main !== module && String(process.env?.[MODE_KEY] || '').trim()) {
  setImmediate(() => {
    runConfiguredAlignment().catch((error) => logger.error({ err: error }, '#779 practitioner alignment control failed closed'));
  });
}
...
module.exports = {
  ...
  sanitizeAccess,
  sanitizeServices,
  assertSourcePractitioner,
  sanitizedSnapshot,
  sameServiceIds,
  alreadyAligned,
  loadSnapshot,
  applyAlignment,
  runConfiguredAlignment,
};
