...
function sanitizeAccess(row) {
  ...
  return {
    ...
    rawStaffScope: String(row.staff_scope || ''), capabilities: enabledCapabilities(row.permissions),
  };
}
...
function assertSourcePractitioner(staff, access) {
  ...
  if (access.calendar_scope !== 'own_appointments' || access.service_scope !== 'own_services') {
    throw new Error('#779 source access is not the canonical own-scope practitioner preset');
  }
  const capabilities = enabledCapabilities(access.permissions);
  if (capabilities.length !== 1 || capabilities[0] !== 'appointment:view') {
    throw new Error('#779 source practitioner does not match the canonical view-only capability preset');
  }
  const forbidden = capabilities.filter((capability) => FORBIDDEN_CAPABILITIES.has(capability));
  ...
  try {
    ...
    await client.query(
      ...
      [before.targetAccess.id, targetStaff.id, before.sourceAccess.role, JSON.stringify(before.sourceAccess.permissions || {}),
        before.sourceAccess.business_role, before.sourceAccess.calendar_scope, before.sourceAccess.service_scope]
    );
    ...
    await client.query(
      ...
      [before.targetAccess.id, JSON.stringify({
        ...
        rawStaffScopeManagedByCanonicalPreset: false,
