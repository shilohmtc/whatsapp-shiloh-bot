...
async function resolveDummyTestApprovalPolicy(db, appointmentId) {
  const appointment = await db.query(`
    SELECT a.client_id, c.display_name,
           (SELECT COUNT(*)::int FROM clients dc WHERE LOWER(TRIM(dc.display_name)) = LOWER($2) AND dc.status = 'active') AS active_dummy_count
      FROM appointments a JOIN clients c ON c.id = a.client_id WHERE a.id = $1
...
async function createPendingBookingApproval(db, { appointmentId, staffId, staffName }) {
  ...
  const result = await db.query(`
    ...
    ON CONFLICT (appointment_id) DO UPDATE SET
      ...
      updated_at = NOW()
    RETURNING appointment_id, approver_staff_id, approver_admin_id, observer_staff_id, status
  `, [appointmentId, approverStaffId, approverAdminId, observerStaffId]);
...
async function approvalContext(appointmentId, db = pool) {
  await ensureBookingApprovalTable(db);
  const result = await db.query(`
    SELECT aba.appointment_id, aba.approver_staff_id, aba.approver_admin_id, aba.observer_staff_id, aba.status,
           ...
           COALESCE((SELECT string_agg(ast.staff_name_snapshot, ' + ' ORDER BY ast.position) FROM appointment_staff ast WHERE ast.appointment_id = a.id), practitioner_approver.display_name, admin_approver.display_name) AS staff_name,
           COALESCE(admin_approver.display_name, practitioner_approver.display_name) AS approver_name,
      ...
      LEFT JOIN staff practitioner_approver ON practitioner_approver.id = aba.approver_staff_id
      LEFT JOIN staff_admin_accounts admin_approver ON admin_approver.id = aba.approver_admin_id AND admin_approver.active = TRUE
...
function approvalRequestBody(context) {
  ...
  return [
    ...
    `Client: ${context.client_name}`, `Treatment: ${context.service_name}`, `With: ${context.staff_name}`, `Time: ${fmtDateTime(context.starts_at)}`, '',
...
async function requestPractitionerApproval({ appointmentId }) {
  ...
  const approver = context.approver_admin_id
    ? await adminContactForAdmin(context.approver_admin_id)
    : await adminContactForStaff(context.approver_staff_id);
  if (!approver) {
...
async function notifyOtherDecisionMaker(context, decision, decidingAdmin) {
  ...
  await sendWhatsAppMessage(other.normalized_whatsapp, [
    '*Abigail booking request update*', '', `${context.client_name} — ${context.service_name} — ${fmtDateTime(context.starts_at)}`,
    `${decidingAdmin.display_name} has ${decision === 'approved' ? 'approved' : 'declined'} the request.`, 'The first valid decision is final for this request.',
  ].join('\n'));
}
...
async function declineBookingRequest(admin, context) {
  ...
  if (phone) {
    try { await sendWhatsAppMessage(phone, ['*Booking request update*', '', `Your request for ${context.service_name} on ${fmtDateTime(context.starts_at)} could not be confirmed.`, 'The held time has been released. Nothing is booked.', '', 'Reply *BOOKING* when you would like to choose another available time. 🌿'].join('\n')); }
