...
function earningsEntries(body = '') {
  return (parseVisibleMenu(body).get('Reports') || []).filter(({ action }) =>
    ['christel_earnings', 'abigail_earnings', 'marietjie_earnings'].includes(action.key));
}
...
function visibleEntriesBySection(body = '') {
  ...
  // Walk-ins are no longer part of operational WhatsApp Admin. Client lookup lives under More.
  ...
  // Reports presents a single Earnings entry; role-aware practitioner choices sit behind it.
  const reports = (sections.get('Reports') || []).filter(({ action }) =>
    !['christel_earnings', 'abigail_earnings', 'marietjie_earnings'].includes(action.key));
...
function sectionInteractive(section, body) {
  ...
  const rows = entries.map(({ action, title }) => ({
    ...
    title: action.key === 'schedule' ? 'Manage schedule' : (title.length <= 24 ? title : title.slice(0, 24)),
...
function earningsInteractive(body) {
  ...
  const rows = entries.map(({ action, title }) => ({
    id: `admin_action_${action.key}`,
    title: title.replace(/^💰\s*/u, '').slice(0, 24),
    description: 'Completed-only earnings report',
  }));
  rows.push({ id: 'admin_section_reports', title: '← Back to Reports', description: 'Return to Reports' });
  return {
    type: 'list',
    body: '*Earnings*\nChoose whose completed-treatment earnings you want to view.',
    buttonText: 'Earnings',
    rows,
...
function isChristelOwnerAdmin(admin) {
  return normalizedAdminName(admin) === 'christel'
    && ['owner', 'business_admin'].includes(admin?.business_role)
    && admin?.calendar_scope === 'all_business';
}
...
function enrichPrivilegedReportsMenu(result) {
  ...
  if (result.admin?.permissions?.['booking:update'] === true
...
async function dispatchStableAction(sender, action) {
  ...
  if (action.key === 'earnings') {
    ...
    if (!menuResult?.handled || !menuResult?.interactive?.body) {
      return { handled: true, admin: menuResult?.admin, reply: 'Admin menu could not be refreshed safely. Send *Menu* to restart Admin.' };
    }
    ...
    return interactive
      ? { handled: true, admin: menuResult.admin, interactive }
      : { handled: true, admin: menuResult.admin, reply: 'No earnings reports are available for your account.' };
  }
...
async function processAdminInteractiveMenuMessage(sender, text) {
  ...
  const testReset = await processAdminTestClientResetMessage(sender, text);
  if (testReset.handled) return testReset;
  ...
  if (action) {
    // Retired walk-in action IDs remain fail-closed even though the low-level owner stays available for legacy/internal use.
    ...
    if (!isActionVisibleInMenu(action, menuResult.interactive.body)) {
      return { handled: true, admin: menuResult.admin, reply: 'That admin action is not available for your account. Send *Menu* to refresh your options.' };
    }
  ...
  if (result?.handled
      ...
      && /^\*Shiloh Admin 🌿\*/.test(result.interactive.body || '')) {
