...
function normalizeLabel(value = '') { return String(value).replace(/[🧪💰🛡️]/gu, '').trim().toLowerCase().replace(/\s+/g, ' '); }
function actionForLabel(label) { const normalized = normalizeLabel(label); return ACTIONS.find((action) => action.labels.some((candidate) => normalizeLabel(candidate) === normalized)) || null; }
function actionForId(id) { const match = String(id || '').trim().match(/^admin_action_([a-z0-9_]+)$/); return match ? ACTIONS.find((action) => action.key === match[1]) || null : null; }
function parseVisibleMenu(body = '') { const sections = new Map(); let currentSection = null; for (const line of String(body).split('\n')) { const sectionMatch = line.trim().match(/^\*(Appointments|Reports|Clients|Services|Schedule|More)\*$/); if (sectionMatch) { currentSection = sectionMatch[1]; if (!sections.has(currentSection)) sections.set(currentSection, []); continue; } const optionMatch = line.trim().match(/^\d+️⃣\s+(.+)$/u); if (!optionMatch || !currentSection) continue; const action = actionForLabel(optionMatch[1]); if (action) sections.get(currentSection).push({ action, title: optionMatch[1].trim() }); } return sections; }
...
function enrichPrivilegedReportsMenu(result) {
  ...
  const jeanPierre = isJeanPierreBusinessAdmin(result.admin); const christel = isChristelOwnerAdmin(result.admin); const marietjie = isMarietjieAdmin(result.admin); let body = String(result.interactive.body);
...
async function getRoleScopedMenu(sender) { const result = await processAdminMobileMenuMessage(sender, 'Menu'); if (!result?.handled || !result?.interactive?.body) return result; return enrichPrivilegedReportsMenu(result); }
async function dispatchStableAction(sender, action) {
  ...
  const privileged = await processJeanPierreControlPlaneMessage(sender, action.command); if (privileged.handled) return privileged; return processAdminMobileMenuMessage(sender, action.command);
}
async function processAdminInteractiveMenuMessage(sender, text) {
  const pendingApprovals = await processAdminPendingBookingApprovalsMessage(sender, text); if (pendingApprovals.handled) return pendingApprovals;
  const finalization = await processAdminAppointmentFinalizationMessage(sender, text); if (finalization.handled) return finalization;
  const testClientReset = await processAdminTestClientResetMessage(sender, text); if (testClientReset.handled) return testClientReset;
  const marietjieEarnings = await processAdminMarietjieEarningsMessage(sender, text); if (marietjieEarnings.handled) return marietjieEarnings;
  const jeanPierreControl = await processJeanPierreControlPlaneMessage(sender, text); if (jeanPierreControl.handled) return jeanPierreControl;
  const raw = String(text || '').trim(); const sectionMatch = raw.match(/^admin_section_(appointments|reports|clients|services|schedule|more)$/i);
  if (sectionMatch) { const menuResult = await getRoleScopedMenu(sender); if (!menuResult?.handled || !menuResult?.interactive?.body) return { handled: true, admin: menuResult?.admin, reply: 'Admin menu could not be refreshed safely. Send *Menu* to restart Admin.' }; const section = SECTION_ORDER.find((value) => value.toLowerCase() === sectionMatch[1].toLowerCase()); const interactive = sectionInteractive(section, menuResult.interactive.body); if (!interactive) return { handled: true, admin: menuResult.admin, reply: 'That admin section is not available for your account. Send *Menu* to refresh your options.' }; return { handled: true, admin: menuResult.admin, interactive }; }
  const action = actionForId(raw);
  if (action) { const menuResult = await getRoleScopedMenu(sender); if (!menuResult?.handled || !menuResult?.interactive?.body) return { handled: true, admin: menuResult?.admin, reply: 'Admin menu could not be refreshed safely. Send *Menu* to restart Admin.' }; if (!isActionVisibleInMenu(action, menuResult.interactive.body)) return { handled: true, admin: menuResult.admin, reply: 'That admin action is not available for your account. Send *Menu* to refresh your options.' }; return dispatchStableAction(sender, action); }
  ...
  const result = enrichPrivilegedReportsMenu(await processAdminMobileMenuMessage(sender, text)); if (result?.handled && result?.interactive?.type === 'button' && /^\*Shiloh Admin 🌿\*/.test(result.interactive.body || '')) return { ...result, interactive: topLevelInteractive(result.interactive.body) }; return result;
}
