...
const TEMPLATE = process.env.WHATSAPP_STAFF_FINALIZATION_TEMPLATE || '';
...
async function reminderRecipients() {
  const result = await pool.query(
    `SELECT saa.id AS admin_id, saa.normalized_whatsapp, saa.display_name, saa.staff_id,
            lower(trim(saa.display_name)) AS admin_name
       FROM staff_admin_accounts saa
      WHERE saa.active=TRUE
        AND lower(trim(saa.display_name)) IN ('christel','abigail','marietjie')
      ORDER BY saa.id`
  );
  return result.rows;
}
...
async function activeStaffIdsByName(names) {
  const result = await pool.query(
    `SELECT id, lower(trim(display_name)) AS name
       FROM staff
      WHERE status='active' AND lower(trim(display_name)) = ANY($1::text[])`,
    [names]
  );
...
async function claimReminder(adminId, clinicDate, kind, pendingCount) {
  const result = await pool.query(
    `INSERT INTO attendance_finalization_reminders
       (admin_id, clinic_date, reminder_kind, pending_count)
     VALUES ($1,$2,$3,$4)
     ON CONFLICT (admin_id, clinic_date, reminder_kind) DO NOTHING
     RETURNING id`,
    [adminId, clinicDate, kind, pendingCount]
  );
  return result.rows[0]?.id || null;
}

async function undoClaim(id) {
  if (!id) return;
  await pool.query(`DELETE FROM attendance_finalization_reminders WHERE id=$1`, [id]);
}

function reminderLabel(kind) {
  return kind === 'next_morning' ? 'still need finalization from the previous clinic day' : 'need finalization from today';
}

async function processAttendanceFinalizationReminders(now = new Date()) {
  if (!TEMPLATE) return { enabled: false, sent: 0, reason: 'template_not_configured' };
  const window = reminderWindow(now);
  if (!window) return { enabled: true, sent: 0, reason: 'outside_window' };

  const [admins, staffMap] = await Promise.all([
    reminderRecipients(),
    activeStaffIdsByName(['christel', 'abigail', 'marietjie']),
  ]);
  let sent = 0;
  for (const admin of admins) {
    ...
    try {
      await sendWhatsAppTemplate(
        ...
        TEMPLATE,
        [admin.display_name, String(pendingCount), window.clinicDate, reminderLabel(window.kind)],
        LANGUAGE_CODE
      );
      sent += 1;
...
async function runScan() {
  if (running) return;
  running = true;
  try { await processAttendanceFinalizationReminders(); }
  catch (error) { logger.error({ err: error }, 'Attendance finalization reminder scan failed'); }
...
function startAttendanceFinalizationReminderScheduler() {
  if (timer) return;
  logger.info({
    enabled: Boolean(TEMPLATE), scanMinutes: SCAN_MINUTES, endOfDayHour: END_OF_DAY_HOUR, nextMorningHour: NEXT_MORNING_HOUR,
  }, 'Attendance finalization reminder scheduler started');
  setTimeout(runScan, 7000).unref();
...
module.exports = {
  ...
  offsetClinicDate,
  reminderWindow,
  pendingCountForAuthority,
  processAttendanceFinalizationReminders,
