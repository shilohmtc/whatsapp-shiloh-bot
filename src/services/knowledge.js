...
async function insertDocumentWithChunks(client, { title, source, content }, chunks) {
...
async function ingestDocument({ title, source, content }) {
  ...
  try {
    ...
    const document = await insertDocumentWithChunks(
      client,
...
async function replaceDocumentBySource({ title, source, content }) {
  await ensureKnowledgeSchema();
  const chunks = chunkText(content);
  if (!title?.trim()) throw new Error("Document title is required");
  if (!source?.trim()) throw new Error("Document source is required for replacement");
  if (chunks.length === 0) throw new Error("Document content is required");

  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    await client.query("DELETE FROM documents WHERE source = $1", [source.trim()]);
    const document = await insertDocumentWithChunks(
      client,
      { title, source, content },
      chunks
    ...
    await client.query("ROLLBACK");
    logger.error({ err: error, source }, "Document source replacement failed");
    throw error;
  } finally {
    client.release();
  }
