...
function appointmentsHelp(admin) {
  ...
  const lines = ["Appointments Help", "", "Read-only:", "• Today", "• Tomorrow", "• Check availability STAFF | SERVICE | DD/MM/YYYY HH:MM", "", "Availability checks staff/service eligibility plus overlapping canonical appointments and staff calendar blocks. It is not yet a recurring staff-working-hours guarantee."];
  if (hasPermission(admin, "appointment:create")) lines.push("", "Guarded booking creation:", "1. Find client [name/number] and use the canonical CRM ID", "2. Check availability STAFF | SERVICE | DD/MM/YYYY HH:MM", "3. Book client CRM_ID | STAFF | SERVICE | DD/MM/YYYY HH:MM", "4. Review Shiloh's summary", "5. Reply exactly CONFIRM BOOKING to write the appointment, or CANCEL BOOKING", "", "Immediately before writing, Shiloh locks the staff booking path and re-checks active records, eligibility and conflicts inside the transaction.");
  return lines.join("\n");
}
