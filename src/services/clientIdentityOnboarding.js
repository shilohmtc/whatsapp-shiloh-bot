...
function extractBundledRegistration(text = "") {
  ...
  const mobileMatch = source.match(/(?:\+?27|0027|0)[\s-]?\d{2}[\s-]?\d{3}[\s-]?\d{4}\b/);
  const dobMatch = source.match(/\b\d{1,2}[\/-]\d{1,2}[\/-]\d{4}\b|\b\d{1,2}\s+[A-Za-z]+,?\s+\d{4}\b/);
  if (!mobileMatch || !dobMatch) return null;

  const mobileNumber = normalizeRegistrationMobile(mobileMatch[0]);
  const dateOfBirth = parseDateOfBirth(dobMatch[0]);
  if (!mobileNumber || !dateOfBirth) return null;

  const nameText = source
    .replace(mobileMatch[0], " ")
    .replace(dobMatch[0], " ")
    .replace(/\b(?:full\s+name|name|mobile(?:\s+number)?|phone(?:\s+number)?|dob|date\s+of\s+birth)\b\s*[:=-]?/gi, " ")
    .replace(/[\n\r,;|]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  const fullName = cleanName(nameText);
  if (!fullName) return null;

  return { fullName, mobileNumber, dateOfBirth };
}
