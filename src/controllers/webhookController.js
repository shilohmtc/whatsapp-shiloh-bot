...
exports.receiveWebhook = async (req, res) => {
  ...
  try {
    ...
    try {
      // Authenticated admin help is routed first so HELP/MENU never becomes walk-in
      // form data or falls through to the normal client conversation pipeline.
      const adminHelp = await processAdminHelpMessage(from, text);
      if (adminHelp.handled) {
        log.info({ from: maskPhone(from), admin: adminHelp.admin?.display_name }, "Handled authenticated admin help request");
        await sendWhatsAppMessage(from, adminHelp.reply);
        return res.sendStatus(200);
      }
      ...
      if (customerExperience.handled) {
        log.info({ from: maskPhone(from), experienceStatus: customerExperience.experience?.status, rating: customerExperience.experience?.rating }, "Handled customer-experience conversation");
      ...
      if (identity.handled) {
        log.info({ from: maskPhone(from), identityStatus: identity.identityStatus, onboardingComplete: Boolean(identity.onboardingComplete) }, "Handled canonical client identity/onboarding conversation");
      ...
      if (identity.identityStatus === "matched_incomplete" && identity.client?.display_name && isGreetingOnly(text)) {
        log.info({ from: maskPhone(from), identityStatus: identity.identityStatus }, "Recognized incomplete canonical client greeting");
      ...
      if (!scope.allowed) {
        log.info({ from: maskPhone(from), scopeReason: scope.reason }, "Redirecting off-topic WhatsApp request");
      ...
      if (appointmentChange.handled) {
        log.info({ from: maskPhone(from), action: appointmentChange.intent?.action, changeStatus: appointmentChange.intent?.status }, "Handled appointment-change conversation");
      ...
      if (booking.handled) {
        log.info({ from: maskPhone(from), bookingStatus: booking.intent?.status }, "Handled booking-intent conversation");
