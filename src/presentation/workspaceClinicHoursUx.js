...
function renderException(exception) {
  ...
  const title = exception.holidayName || exception.reason || 'Clinic-wide exception';
  const summary = open
    ? `Open ${escapeHtml(exception.startsLocal || '')}–${escapeHtml(exception.endsLocal || '')}`
    : 'Closed all day';
  const reason = exception.reason && exception.reason !== exception.holidayName
    ? `<span class="exception-meta">${escapeHtml(exception.reason)}</span>`
    : '';
  return `<div class="exception-row" data-clinic-exception data-date="${escapeHtml(exception.exceptionDate)}" data-type="${open ? 'open' : 'closed'}" data-start="${escapeHtml(exception.startsLocal || '')}" data-end="${escapeHtml(exception.endsLocal || '')}" data-reason="${escapeHtml(exception.reason || '')}">
    ...
    <div class="exception-summary">${summary}${reason}</div>
...
function renderClinicHoursPage(model, {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Clinic hours — Shiloh Workspace</title><style>${workspaceShellStyles()}${clinicHoursStyles()}</style><script src="${escapeHtml(staffAccessScriptPath)}" defer></script><script src="${escapeHtml(clientScriptPath)}" defer></script></head><body data-workspace-clinic-hours="true"><div class="workspace-frame">${renderWorkspaceNavigation({
    ...
    <section class="notice"><strong>Clinic-wide authority:</strong> these settings shape Calendar, booking and reschedule availability. They do not create, remove or alter practitioner leave or practitioner blocks.</section>
    ...
    <section class="exception-card" aria-labelledby="clinic-exceptions-title">
      ...
      <form data-clinic-exception-form novalidate>
        <div class="exception-grid">
          ...
          <label><span class="field-note">Reason / label</span><input class="text-input" data-exception-reason maxlength="240" placeholder="Public holiday, clinic closure…"></label>
