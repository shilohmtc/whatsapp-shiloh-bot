...
function compactWeekLabel(dateKeys = []) {
  ...
  const firstDay = new Intl.DateTimeFormat('en-ZA', { timeZone: BUSINESS_TIMEZONE, day: 'numeric' }).format(first);
  const lastDay = new Intl.DateTimeFormat('en-ZA', { timeZone: BUSINESS_TIMEZONE, day: 'numeric' }).format(last);
  const firstMonth = new Intl.DateTimeFormat('en-ZA', { timeZone: BUSINESS_TIMEZONE, month: 'short' }).format(first);
  const lastMonth = new Intl.DateTimeFormat('en-ZA', { timeZone: BUSINESS_TIMEZONE, month: 'short' }).format(last);
  const year = new Intl.DateTimeFormat('en-ZA', { timeZone: BUSINESS_TIMEZONE, year: 'numeric' }).format(last);
...
body.dataset.calendarDesktopApproved='true';
...
const params=new URLSearchParams(location.search);const view=body.dataset.calendarView||params.get('view')||'week';
...
function compactLabel(keys){if(!keys.length)return'';const parse=key=>new Date(key+'T12:00:00+02:00');const first=parse(keys[0]),last=parse(keys[keys.length-1]);const day=date=>new Intl.DateTimeFormat('en-ZA',{timeZone:'Africa/Johannesburg',day:'numeric'}).format(date);const month=date=>new Intl.DateTimeFormat('en-ZA',{timeZone:'Africa/Johannesburg',month:'short'}).format(date);const year=new Intl.DateTimeFormat('en-ZA',{timeZone:'Africa/Johannesburg',year:'numeric'}).format(last);return month(first)===month(last)?day(first)+'–'+day(last)+' '+month(last)+' '+year:day(first)+' '+month(first)+'–'+day(last)+' '+month(last)+' '+year;}
function installStyles(){const style=document.createElement('style');style.dataset.calendarDesktopApprovedStyles='true';style.textContent=`
...
body[data-calendar-desktop-approved="true"] .desktop-calendar-toolbar .nav-button,body[data-calendar-desktop-approved="true"] .desktop-calendar-toolbar .view-tab{min-height:36px;border-radius:9px;padding:6px 10px}
...
body[data-calendar-desktop-approved="true"] .desktop-create-menu>summary{display:inline-flex;align-items:center;justify-content:center;min-height:38px;padding:7px 13px;border:1px solid #17382d;border-radius:9px;background:#17382d;color:#fff;font-size:.8rem;font-weight:850;list-style:none;cursor:pointer;white-space:nowrap}
body[data-calendar-desktop-approved="true"] .desktop-create-menu>summary::-webkit-details-marker{display:none}
...
body[data-calendar-desktop-approved="true"] .desktop-create-popover>a,body[data-calendar-desktop-approved="true"] .desktop-create-popover>button,body[data-calendar-desktop-approved="true"] .desktop-create-submenu>summary{display:flex;align-items:center;width:100%;min-height:38px;padding:8px 10px;border:0;border-radius:8px;background:transparent;color:#20322b;font:inherit;font-size:.78rem;font-weight:750;text-align:left;cursor:pointer;text-decoration:none;list-style:none}
body[data-calendar-desktop-approved="true"] .desktop-create-popover>a:hover,body[data-calendar-desktop-approved="true"] .desktop-create-popover>button:hover,body[data-calendar-desktop-approved="true"] .desktop-create-submenu>summary:hover{background:#e7eee9}
body[data-calendar-desktop-approved="true"] .desktop-create-submenu>summary::-webkit-details-marker{display:none}
...
function buildToolbar(days,selected){const controls=one('.controls');if(!controls)return null;const period=one('.period-nav',controls),tabs=one('.view-tabs',controls),today=one('.today',controls);if(!period||!tabs||!today)return null;const context=one('.period-context',period);if(context&&days.length)context.textContent=compactLabel(days);today.href=calendarHref('week',businessToday());const toolbar=document.createElement('section');toolbar.className='desktop-calendar-toolbar';toolbar.setAttribute('aria-label','Calendar navigation');toolbar.append(period,tabs,today);const menu=document.createElement('details');menu.className='desktop-create-menu';menu.innerHTML='<summary>+ New appointment</summary><div class="desktop-create-popover" data-desktop-create-popover></div>';toolbar.append(menu);controls.parentNode.insertBefore(toolbar,controls);const originalActions=all('.operational-actions a');const popover=one('[data-desktop-create-popover]',menu);originalActions.forEach(link=>{const clone=link.cloneNode(true);clone.className='';clone.textContent=link.textContent.replace(/^\+\s*/,'');popover.append(clone);});all('.operational-actions').forEach(node=>node.remove());return {menu,popover};}
