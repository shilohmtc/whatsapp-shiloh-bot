...
async function register(){msg('Preparing secure passkey setup…');if(!window.PublicKeyCredential||!navigator.credentials){msg('Passkeys are not supported in this browser.');return;}add.disabled=true;try{await ensureCsrf();var r=await fetch('/calendar/staff-auth/passkeys/registration/options',{method:'POST',credentials:'same-origin',cache:'no-store',headers:headers(),body:'{}'});var b=await safe(r);if(r.status===428){msg('Sign in again with your authenticator or an existing passkey before adding a passkey.');return;}if(r.status===403){msg('Secure passkey setup was rejected. Reopen Shiloh and try again.');return;}if(!r.ok)throw new Error('start');var o=b.options;o.challenge=bytes(o.challenge);o.user.id=bytes(o.user.id);(o.excludeCredentials||[]).forEach(function(c){c.id=bytes(c.id);});msg('Approve passkey creation on this device…');var c=await navigator.credentials.create({publicKey:o});if(!c)throw new Error('cancelled');var transports=c.response.getTransports?c.response.getTransports():[];var finish=await fetch('/calendar/staff-auth/passkeys/registration/finish',{method:'POST',credentials:'same-origin',cache:'no-store',headers:headers(),body:JSON.stringify({response:{id:c.id,rawId:enc(c.rawId),type:c.type,response:{clientDataJSON:enc(c.response.clientDataJSON),attestationObject:enc(c.response.attestationObject),transports:transports}}})});if(!finish.ok)throw new Error('finish');msg('Passkey added.');await load();}catch(error){if(error&&error.name==='NotAllowedError')msg('Passkey registration was cancelled or unavailable.');else if(error&&error.name==='SecurityError')msg('This installed app cannot start passkey setup on the current origin.');else msg('Passkey could not be added.');}finally{add.disabled=false;}}
