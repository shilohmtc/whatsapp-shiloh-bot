...
function accessPolicyMarkup(model = {}) {
  ...
  return `<div data-staff-access-management data-access-policy-editor><p class="status-message" role="status" aria-live="polite" data-staff-access-status></p><div class="access-policy-scope" aria-label="Fixed practitioner authority"><div><span>Business role</span><strong>${escapeHtml(humanScope(policy.businessRole))}</strong></div><div><span>Calendar scope</span><strong>${escapeHtml(humanScope(policy.calendarScope))}</strong></div><div><span>Service scope</span><strong>${escapeHtml(humanScope(policy.serviceScope))}</strong></div></div><h3 class="access-policy-heading">Operational capabilities</h3><form data-staff-access-policy-form data-staff-id="${escapeHtml(model.staff?.id)}" data-access-revision="${escapeHtml(policy.revision)}"><div class="access-policy-options">${controls}</div>${policy.key==='workspace_retrospective_access_v1'?`<div class="field wide"><label for="retrospective-client-ids">Canonical client IDs (optional restriction)</label><input id="retrospective-client-ids" name="retrospectiveClientIds" inputmode="numeric" value="${escapeHtml(Array.isArray(policy.retrospectiveClientIds)?policy.retrospectiveClientIds.join(', '):'')}" placeholder="Leave blank for existing-scope access"><span class="policy-copy">Comma-separated canonical CRM V2 IDs. Existing staff, service and location scopes still apply.</span></div>`:''}<p class="access-policy-help"><strong>Complete / No-show visits</strong> is available because canonical finalization now proves the linked practitioner identity, own-appointment scope and every assignment row; unresolved or NULL assignment evidence fails closed. Other mutation capabilities remain outside this editor until their own-scope contracts are separately proven.</p><div class="manage-actions"><button class="button primary" type="submit">Save access</button><span class="manage-badge">Practitioner policy</span></div></form><div class="read-only-note">This surface does not change role, calendar scope, service scope, WhatsApp identity, credentials, owner/admin authority or protected security capabilities.</div></div>`;
}
...
function accessEnablementMarkup(model = {}) {
  ...
  if (model.access) {
    ...
    const reason = model.accessPolicy?.reason
      ...
      : '';
    return `<div data-staff-access-readonly>${READ_ONLY_NOTE}${reason}<p class="footer-note">Existing access is preserved. Protected, broader or incompatible authority is not silently downgraded or replaced.</p></div>`;
  }
