...
const READ_ONLY_NOTE = '<div class="read-only-note">Access settings are view only here. Staff profile changes do not create or edit sign-in credentials.</div>';
...
function accessPolicyMarkup(model = {}) {
  ...
  return `<div data-staff-access-management data-access-policy-editor><p class="status-message" role="status" aria-live="polite" data-staff-access-status></p><div class="access-policy-scope" aria-label="Fixed practitioner access"><div><span>Business role</span><strong>${escapeHtml(humanScope(policy.businessRole))}</strong></div><div><span>Calendar access</span><strong>${escapeHtml(humanScope(policy.calendarScope))}</strong></div><div><span>Service access</span><strong>${escapeHtml(humanScope(policy.serviceScope))}</strong></div></div><h3 class="access-policy-heading">Workspace access</h3><form data-staff-access-policy-form data-staff-id="${escapeHtml(model.staff?.id)}" data-access-revision="${escapeHtml(policy.revision)}"><div class="access-policy-options">${controls}</div>${policy.key==='workspace_retrospective_access_v1'?`<div class="field wide"><label for="retrospective-client-ids">Client record IDs (optional restriction)</label><input id="retrospective-client-ids" name="retrospectiveClientIds" inputmode="numeric" value="${escapeHtml(Array.isArray(policy.retrospectiveClientIds)?policy.retrospectiveClientIds.join(', '):'')}" placeholder="Leave blank for existing access"><span class="policy-copy">Comma-separated client record IDs. Existing staff, service and location access still applies.</span></div>`:''}<p class="access-policy-help"><strong>Complete / No-show visits</strong> is available only when this staff member’s appointment access and assignment are valid. Other booking and schedule changes are managed separately.</p><div class="manage-actions"><button class="button primary" type="submit">Save access</button><span class="manage-badge">Practitioner access</span></div></form><div class="read-only-note">This page does not change role, Calendar access, service access, WhatsApp identity, sign-in credentials, owner/admin access or protected security settings.</div></div>`;
}
...
function accessEnablementMarkup(model = {}) {
  ...
  if (staff.resource_type !== 'practitioner' || staff.business_role !== 'employee_practitioner') {
    return `${READ_ONLY_NOTE}<div class="warning-note">This access setup is only available for active employee practitioners. Other staff access is managed separately.</div>`;
  }
...
var accessForm=one('[data-staff-access-enable-form]');if(accessForm)accessForm.addEventListener('submit',async function(e){e.preventDefault();var f=new FormData(accessForm);var completion=accessForm.dataset.accessMode==='complete';if(f.get('identityConfirmed')!=='on'){status('Verify the staff member’s current WhatsApp number before changing access.','error');return;}busy(accessForm,true);status(completion?'Completing Workspace access…':'Enabling Workspace access…','working');try{var token=await csrf();var suffix=completion?COMPLETE_SUFFIX:ENABLE_SUFFIX;var response=await fetch(API+'/'+accessForm.dataset.staffId+suffix,{method:'POST',credentials:'same-origin',cache:'no-store',headers:{'Content-Type':'application/json','Accept':'application/json','x-shiloh-csrf-token':token},body:JSON.stringify({requestId:requestId(),expectedRevision:accessForm.dataset.staffRevision,whatsappNumber:f.get('whatsappNumber'),identityConfirmed:true})});token='';var body=await json(response);if(!response.ok)throw new Error(body.error||'Workspace access change could not be completed.');window.location.reload();}catch(err){status(err.message,'error');busy(accessForm,false);}});
var policyForm=one('[data-staff-access-policy-form]');if(policyForm)policyForm.addEventListener('submit',async function(e){e.preventDefault();var capabilities=Array.prototype.filter.call(policyForm.querySelectorAll('input[name="capability"]'),function(input){return input.checked;}).map(function(input){return input.value;});busy(policyForm,true);status('Saving practitioner access…','working');try{var token=await csrf();var response=await fetch(API+'/'+policyForm.dataset.staffId+POLICY_SUFFIX,{method:'POST',credentials:'same-origin',cache:'no-store',headers:{'Content-Type':'application/json','Accept':'application/json','x-shiloh-csrf-token':token},body:JSON.stringify({requestId:requestId(),expectedAccessRevision:policyForm.dataset.accessRevision,capabilities:capabilities,retrospectiveClientIds:(one('#retrospective-client-ids')?(one('#retrospective-client-ids').value.trim()?one('#retrospective-client-ids').value.split(',').map(function(v){return v.trim();}).filter(Boolean):null):undefined)})});token='';var body=await json(response);if(!response.ok)throw new Error(body.error||'Staff access change could not be completed.');window.location.reload();}catch(err){status(err.message,'error');busy(policyForm,false);}});
})();`;
}
