...
var form=one('[data-staff-access-enable-form]');if(!form)return;form.addEventListener('submit',async function(e){e.preventDefault();var f=new FormData(form);var completion=form.dataset.accessMode==='complete';if(f.get('identityConfirmed')!=='on'){status('Verify the staff member’s current WhatsApp number before changing access.','error');return;}var action=completion?'Complete view-only Shiloh Workspace access for this staff member using the verified existing WhatsApp identity?':'Enable view-only Shiloh Workspace access for this staff member using the verified WhatsApp number?';if(!window.confirm(action))return;busy(form,true);status(completion?'Completing canonical Workspace access…':'Enabling canonical Workspace access…','working');try{var token=await csrf();var suffix=completion?COMPLETE_SUFFIX:ENABLE_SUFFIX;var response=await fetch(API+'/'+form.dataset.staffId+suffix,{method:'POST',credentials:'same-origin',cache:'no-store',headers:{'Content-Type':'application/json','Accept':'application/json','x-shiloh-csrf-token':token},body:JSON.stringify({requestId:requestId(),expectedRevision:form.dataset.staffRevision,whatsappNumber:f.get('whatsappNumber'),identityConfirmed:true})});token='';var body=await json(response);if(!response.ok)throw new Error(body.error||'Workspace access change failed closed.');window.location.reload();}catch(err){status(err.message,'error');busy(form,false);}});
})();`;
}
