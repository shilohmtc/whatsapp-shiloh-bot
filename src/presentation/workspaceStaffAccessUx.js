...
function accessPolicyMarkup(model = {}) {
  ...
  return `<div data-staff-access-management data-access-policy-editor><p class="status-message" role="status" aria-live="polite" data-staff-access-status></p><div class="access-policy-scope" aria-label="Fixed practitioner authority"><div><span>Business role</span><strong>${escapeHtml(humanScope(policy.businessRole))}</strong></div><div><span>Calendar scope</span><strong>${escapeHtml(humanScope(policy.calendarScope))}</strong></div><div><span>Service scope</span><strong>${escapeHtml(humanScope(policy.serviceScope))}</strong></div></div><h3 class="access-policy-heading">Operational access</h3><form data-staff-access-policy-form data-staff-id="${escapeHtml(model.staff?.id)}" data-access-revision="${escapeHtml(policy.revision)}"><div class="access-policy-options">${controls}</div><p class="access-policy-help">Workspace access remains limited to this practitioner’s own appointments and own services. Only actions whose canonical authority is proven to stay within that boundary are editable here.</p><div class="manage-actions"><button class="button primary" type="submit">Save access</button><span class="manage-badge">Practitioner access</span></div></form><div class="read-only-note">Authentication, WhatsApp identity, owner/admin authority and protected security capabilities are not changed by this surface.</div></div>`;
}
