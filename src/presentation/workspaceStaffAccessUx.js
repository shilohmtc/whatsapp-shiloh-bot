...
var form=one('[data-staff-access-enable-form]');if(!form)return;form.addEventListener('submit',async function(e){e.preventDefault();var f=new FormData(form);if(!window.confirm('Enable view-only Shiloh Workspace access for this staff member using the entered WhatsApp number?'))return;busy(form,true);status('Enabling canonical Workspace access…','working');try{var token=await csrf();var response=await fetch(API+'/'+form.dataset.staffId+'/access/enable',{method:'POST',credentials:'same-origin',cache:'no-store',headers:{'Content-Type':'application/json','Accept':'application/json','x-shiloh-csrf-token':token},body:JSON.stringify({requestId:requestId(),expectedRevision:form.dataset.staffRevision,whatsappNumber:f.get('whatsappNumber')})});token='';var body=await json(response);if(!response.ok)throw new Error(body.error||'Workspace access enablement failed closed.');window.location.reload();}catch(err){status(err.message,'error');busy(form,false);}});
})();`;
}
