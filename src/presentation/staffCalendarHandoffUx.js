...
if(!match){setStatus('invalid','That Workspace link is missing or invalid. Send Open Workspace in Shiloh to get a new link.');return;}
var token=match[1];
...
setStatus('pending','Opening your secure Shiloh Workspace…');
postJson(AUTH_BASE+'/calendar-handoff/exchange',{token:token}).then(function(response){
  token='';
  if(response.status===200){
    setStatus('authenticated','Secure handoff accepted. Opening Shiloh Workspace…');
    window.location.replace(CALENDAR_PATH);
    return;
  }
  if(response.status===401||response.status===403){setStatus('invalid','That Workspace link is invalid, expired, already used, or no longer authorized. Send Open Workspace in Shiloh to get a new link.');return;}
  setStatus('provider-unavailable','Secure Workspace access is temporarily unavailable. Send Open Workspace in Shiloh and try again.');
}).catch(function(){
  token='';
  setStatus('provider-unavailable','Secure Workspace access is temporarily unavailable. Check your connection and try again.');
});
})();`;
}
