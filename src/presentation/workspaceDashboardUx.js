...
function renderDashboardPage(model, { staffAccessScriptPath = '/calendar/staff/client.js', dashboardScriptPath = '/calendar/workspace/client.js', navigation = {} } = {}) {
  ...
  const carryOverCards = carryOver.map(item => appointmentItem(item, model)).join('') || '<div class="empty">No unfinished visits are carrying over from the previous clinic day.</div>';
