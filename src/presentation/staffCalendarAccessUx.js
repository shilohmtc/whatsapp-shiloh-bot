...
function renderStaffCalendarAccessPage({ reason = null, clientScriptPath = '/calendar/staff/client.js', providerIndependentAuthEnabled = false } = {}) {
  ...
  const providerIndependentPanel = providerIndependentAuthEnabled ? `
      <section class="code-panel" data-shiloh-provider-independent-auth>
        <span class="eyebrow">Provider-independent sign-in</span><h2>Use your authenticator</h2>
        ...
        <details><summary>Use a recovery code</summary>
          <p class="lead">A recovery code works once. After sign-in, Shiloh will require authenticator replacement before Calendar access.</p>
...
async function verifyTotp(event){
  ...
  try{
    ...
    if(response.status===200){var body=await safeJson(response);if(viewerPermitsCalendar(body.viewer)){setStatus('authenticated','Sign-in successful. Opening Shiloh Calendar…');window.setTimeout(function(){window.location.assign(CALENDAR_PATH);},350);return;}}
