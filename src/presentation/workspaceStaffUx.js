...
function renderStaffDetailPage(model, options = {}) {
  ...
  const capabilities = access?.capabilities?.length ? access.capabilities.map(cap => `<span class="capability-chip">${escapeHtml(capabilityLabel(cap))}</span>`).join('') : '<span class="muted">No enabled Workspace access is linked.</span>';
