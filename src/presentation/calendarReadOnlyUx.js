...
function eventMeta(item, model) {
  ...
  const pieces = [];
  if (item.kind === 'appointment' && item.serviceName) pieces.push(item.serviceName);
...
function renderCalendarPage(model, {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Shiloh Calendar</title><style>${styles()}</style><script src="${escapeHtml(staffAccessScriptPath)}" defer></script></head><body data-calendar-readonly="true"><div class="shell">
    <header class="topbar"><div class="brand"><h1>Shiloh Calendar</h1><p>Canonical scheduling truth, calmly presented.</p></div><div class="topbar-side"><div class="truth-note">Africa/Johannesburg • Read-only • Shiloh is the scheduling authority</div><div class="access-controls"><button class="signout-button" type="button" data-shiloh-logout>Sign out</button><span class="access-status" role="status" aria-live="polite" data-shiloh-calendar-access-status></span></div></div></header>
