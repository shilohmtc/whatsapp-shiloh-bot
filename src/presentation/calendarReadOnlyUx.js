...
function renderControls(model, basePath) {
  ...
  const filterLinks = [
    ...
    ...(model.permittedStaff || []).map(person => `<a class="filter ${Number(selected) === Number(person.id) ? 'active' : ''}" href="${escapeHtml(queryHref(basePath, model.view, model.dateKey, person.id))}">${escapeHtml(person.displayName)}</a>`),
  ].join('');
  ...
  return `<section class="controls" aria-label="Calendar controls">
    ...
    <div class="filters" aria-label="Practitioner filter">${filterLinks}</div>
  </section>`;
}
