...
function renderReportsPage(model, {
  ...
  const capacityRows = (model.capacity || []).map(row => `<tr>
    <td data-label="Practitioner"><span class="person">${escapeHtml(row.name)}</span></td>
    <td data-label="Scheduled">${escapeHtml(formatMinutes(row.scheduledMinutes))}</td>
    <td data-label="Booked">${escapeHtml(formatMinutes(row.bookedMinutes))}</td>
    ...
    <td data-label="Remaining">${escapeHtml(formatMinutes(row.remainingMinutes))}</td>
    <td data-label="Use"><div class="util"><div class="util-track" aria-hidden="true"><div class="util-fill" style="width:${Math.max(0, Math.min(100, Number(row.utilisationPct || 0)))}%"></div></div><small>${escapeHtml(row.utilisationPct)}%</small></div></td>
  </tr>`).join('');
