...
function installGuidance(){if(!iosDevice()||standalone()||document.querySelector('[data-shiloh-ios-install]'))return;const card=document.createElement('section');card.setAttribute('data-shiloh-ios-install','');card.setAttribute('role','note');card.style.cssText='margin:14px auto 0;width:min(calc(100% - 28px),680px);padding:16px 18px;border:1px solid #cfdad3;border-radius:16px;background:#f4f8f5;color:#20322b;box-shadow:0 6px 20px rgba(20,45,35,.08);font:400 .94rem/1.45 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';const eyebrow=document.createElement('div');eyebrow.textContent='INSTALL SHILOH';eyebrow.style.cssText='margin-bottom:5px;color:#496b5a;font-size:.72rem;font-weight:800;letter-spacing:.12em';const title=document.createElement('strong');title.textContent='Add Shiloh to this iPhone';title.style.cssText='display:block;margin-bottom:5px;font-size:1.05rem';const copy=document.createElement('div');copy.textContent='Tap Share, choose Add to Home Screen, then tap Add. After that, open Shiloh from the new Home Screen icon.';card.append(eyebrow,title,copy);const target=document.body.firstElementChild;document.body.insertBefore(card,target||null);}
function installAction(){if(iosDevice()||standalone()||!deferredInstallPrompt)return;let card=document.querySelector('[data-shiloh-browser-install]');if(card)return;card=document.createElement('section');card.setAttribute('data-shiloh-browser-install','');card.setAttribute('role','note');card.style.cssText='margin:14px auto 0;width:min(calc(100% - 28px),680px);padding:16px 18px;border:1px solid #cfdad3;border-radius:16px;background:#f4f8f5;color:#20322b;box-shadow:0 6px 20px rgba(20,45,35,.08);font:400 .94rem/1.45 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';const eyebrow=document.createElement('div');eyebrow.textContent='INSTALL SHILOH';eyebrow.style.cssText='margin-bottom:5px;color:#496b5a;font-size:.72rem;font-weight:800;letter-spacing:.12em';const title=document.createElement('strong');title.textContent='Install Shiloh on this device';title.style.cssText='display:block;margin-bottom:8px;font-size:1.05rem';const button=document.createElement('button');button.type='button';button.textContent='Install Shiloh';button.style.cssText='min-height:44px;border:1px solid #496b5a;border-radius:999px;padding:9px 15px;background:#496b5a;color:#fff;font:700 .9rem/1.2 system-ui,sans-serif;cursor:pointer';button.addEventListener('click',async()=>{const prompt=deferredInstallPrompt;if(!prompt)return;deferredInstallPrompt=null;removeInstallCard('[data-shiloh-browser-install]');await prompt.prompt();const choice=await prompt.userChoice.catch(()=>null);if(choice&&choice.outcome!=='accepted')deferredInstallPrompt=prompt;installAction();});card.append(eyebrow,title,button);const target=document.body.firstElementChild;document.body.insertBefore(card,target||null);}
...
installGuidance();
addEventListener('beforeinstallprompt',event=>{if(standalone()||iosDevice())return;event.preventDefault();deferredInstallPrompt=event;installAction();});
