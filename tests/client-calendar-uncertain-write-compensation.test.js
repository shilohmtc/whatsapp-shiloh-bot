...
const SLOT = { staffName: 'Christel', startsAt: '2026-08-20T08:00:00Z', endsAt: '2026-08-20T09:00:00Z' };
...
test('rolled-back provider attempt is deleted before a later same-slot write can proceed', async () => {
  const db = fakeDb();
  const cancelled = [];
  const result = await reconcilePendingCalendarWrites({ provider: 'shared_google', currentAppointmentId: 42, ...SLOT, db, cancelEvent: async (eventId, calendarId) => cancelled.push({ eventId, calendarId }) });
  ...
  assert.ok(db.queries.some((entry) => entry.params[2] === SLOT.staffName && entry.params[3] === SLOT.startsAt && entry.params[4] === SLOT.endsAt));
});

test('provider cleanup uncertainty fails closed and leaves the attempt unresolved', async () => {
  const db = fakeDb();
  await assert.rejects(reconcilePendingCalendarWrites({ provider: 'shared_google', currentAppointmentId: 42, ...SLOT, db, cancelEvent: async () => { throw new Error('network uncertain'); } }), (error) => error.code === 'CALENDAR_RECONCILIATION_PENDING');
  assert.ok(db.queries.some((entry) => entry.sql.includes('last_error')));
  assert.ok(!db.queries.some((entry) => entry.params[1] === 'rolled_back_attempt_cleaned'));
});
...
test('a committed canonical appointment is never deleted during recovery', async () => {
  ...
  let cancelCalls = 0;
  const result = await reconcilePendingCalendarWrites({ provider: 'shared_google', currentAppointmentId: 42, ...SLOT, db, cancelEvent: async () => { cancelCalls += 1; } });
  assert.equal(cancelCalls, 0);
  assert.equal(result.reconciled, 1);
  assert.ok(db.queries.some((entry) => entry.params[1] === 'canonical_appointment_exists'));
});

test('recovery refuses an unscoped provider cleanup', async () => {
  const db = fakeDb();
  await assert.rejects(reconcilePendingCalendarWrites({ provider: 'shared_google', currentAppointmentId: 42, db, cancelEvent: async () => {} }), /slot-scoped/);
});
