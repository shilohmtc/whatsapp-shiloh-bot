...
function fakeDb({ canonicalAppointmentExists = false } = {}) {
  ...
  return {
    ...
    async query(sql, params = []) {
      ...
      if (sql.includes('SELECT id, provider, calendar_id')) {
        return {
          rowCount: 1,
          rows: [{ id: 7, provider: 'shared_google', calendar_id: 'calendar-a', event_id: 'event-a', appointment_id: 41 }],
        };
      }
      if (sql.includes('SELECT 1 FROM appointments')) {
        return { rowCount: canonicalAppointmentExists ? 1 : 0, rows: canonicalAppointmentExists ? [{ '?column?': 1 }] : [] };
      }
...
test('rolled-back provider attempt is deleted before a later calendar write can proceed', async () => {
  const db = fakeDb();
  const cancelled = [];
  const result = await reconcilePendingCalendarWrites({
    provider: 'shared_google',
    currentAppointmentId: 42,
    db,
    cancelEvent: async (eventId, calendarId) => cancelled.push({ eventId, calendarId }),
  });

  assert.deepEqual(cancelled, [{ eventId: 'event-a', calendarId: 'calendar-a' }]);
  assert.equal(result.reconciled, 1);
  assert.ok(db.queries.some((entry) => entry.sql.includes('rolled_back_attempt_cleaned')));
});
...
test('provider cleanup uncertainty fails closed and leaves the attempt unresolved', async () => {
  ...
  await assert.rejects(
    reconcilePendingCalendarWrites({
      ...
      currentAppointmentId: 42,
      db,
      cancelEvent: async () => { throw new Error('network uncertain'); },
    }),
    (error) => error.code === 'CALENDAR_RECONCILIATION_PENDING'
  );

  assert.ok(db.queries.some((entry) => entry.sql.includes('SET last_error = $2')));
  assert.ok(!db.queries.some((entry) => entry.sql.includes("resolution = $2") && entry.params[1] === 'rolled_back_attempt_cleaned'));
});
...
test('a committed canonical appointment is never deleted during recovery', async () => {
  ...
  const result = await reconcilePendingCalendarWrites({
    ...
    currentAppointmentId: 42,
    db,
    cancelEvent: async () => { cancelCalls += 1; },
  });
...
test('both shared and practitioner creation wrappers persist recovery state before provider create', () => {
  ...
  assert.ok(shared.includes("provider: 'shared_google'"));
  assert.ok(shared.indexOf('prepareCalendarWrite') < shared.indexOf('createBookingEventOnCalendar(requireCalendarConfig().calendarId,data)') || shared.includes('return createBookingEventOnCalendar(calendarId,data)'));
  assert.ok(practitioner.includes("provider: 'practitioner_google'"));
