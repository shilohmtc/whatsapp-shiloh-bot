...
test('Find a client enters a short-lived guided lookup state', () => {
  ...
  assert.match(source, /step: 'client_lookup'/);
  assert.match(source, /expiresAt: Date\.now\(\) \+ CLIENT_LOOKUP_TTL_MS/);
...
test('guided lookup accepts a bare name, Find name, Find client name, or mobile input', () => {
  ...
  assert.match(source, /reply: await scopedClientLookup\(admin, query\)/);
...
test('guided lookup remains permission scoped and exits cleanly', () => {
  ...
  assert.match(source, /\['menu', 'home'\]\.includes\(value\)/);
  assert.match(source, /value === '0' \|\| value === 'back' \|\| value === 'cancel'/);
  assert.match(source, /moreSessions\.delete\(key\)/);
});
