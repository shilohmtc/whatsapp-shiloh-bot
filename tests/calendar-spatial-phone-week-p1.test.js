...
test('authenticated browser proof mounts canonical Calendar and Create Booking router factories', () => {
  ...
  assert.match(source, /phone-day-empty-time-prefill/);
  assert.match(source, /phone-week-empty-time-prefill/);
  assert.match(source, /phone-compact-direct-drawer/);
  assert.match(source, /phone-plus-actions/);
  assert.match(source, /phone-week-six-days-one-practitioner/);
  assert.match(source, /phone-week-switched-practitioner/);
  assert.match(source, /phone-appointment-manage-sheet/);
  assert.match(source, /calendarViewportShare >= 0\.82/);
  assert.match(source, /data-week-practitioner-lane/);
  assert.match(source, /phone-month-readable-density/);
