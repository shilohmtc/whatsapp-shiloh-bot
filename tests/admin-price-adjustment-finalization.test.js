...
test('price adjustment is a Christel-only historical finalization option', () => {
  assert.match(finalization, /function isChristel\(admin\)/);
  assert.match(finalization, /if \(isChristel\(admin\)\)/);
  assert.match(finalization, /title: 'Price adjustment'/);
  assert.match(finalization, /finalize_price_adjust_/);
  assert.match(finalization, /Price adjustment is reserved for Christel/);
});
...
test('adjusted finalization preserves original value and atomically writes completed financial truth', () => {
  ...
  assert.match(finalization, /priceAuthority: 'christel_discretion'/);
  assert.match(finalization, /COMMIT/);
  assert.match(finalization, /ROLLBACK/);
});

test('adjusted completed value becomes the existing Christel earnings basis', () => {
