...
test('specialty rows keep the approved clinical grouping order', () => {
  const html = renderBookingPage('27823269871', catalogue);

  const profosma = html.indexOf('Profosma Jet Plasma</h2>');
  const consultation = html.indexOf('Plasma Fibroblast Consultation</h2>');
  assert.ok(profosma >= 0 && consultation > profosma);

  const prices = html.indexOf('Plasma Fibroblast Prices</h2>');
  const ozone = html.indexOf('Ozone &amp; Far Infrared</h2>');
  assert.ok(prices >= 0 && ozone > prices);

  const hifu = html.indexOf('HIFU</h2>');
  const vaginal = html.indexOf('Vaginal Tightening &amp; Rejuvenation</h2>');
  const neo = html.indexOf('Neo Pelvic Therapy</h2>');
  assert.ok(hifu >= 0 && vaginal > hifu && neo > vaginal, 'HIFU should be followed by Vaginal Tightening & Rejuvenation, then Neo Pelvic Therapy');
});
