...
const FILENAME = '081_calendar_operational_assurance_cutover.sql';
...
async function main() {
  ...
  if (!status?.applied || status.checksumMatches !== true) {
    throw new Error(`Calendar operational assurance migration failed verification: ${FILENAME}`);
  }
  console.log(JSON.stringify({
    event: 'calendar_operational_assurance_schema_verified',
    filename: FILENAME,
...
main()
  .then(() => pool.end())
  .catch(async (error) => {
    console.error(JSON.stringify({ event: 'calendar_operational_assurance_schema_failed', filename: FILENAME, message: error.message }));
