...
async function logTopManualQueue(){try{const r=await getManualQueue({clientBatchId:"1",appointmentBatchId:"2"});logger.info({manualQueueReview:{summary:r.summary,top:r.items.slice(0,10).map(i=>({queueId:i.queueId,displayName:i.displayName,reason:i.reason,candidateClientId:i.candidateClientId,priority:i.priority,exactAppointmentCount:i.exactAppointmentCount,nearAppointmentCount:i.nearAppointmentCount,appointmentEvidence:i.appointmentEvidence,secondPassClassification:i.secondPassClassification,secondPassConfidence:i.secondPassConfidence,supportedActions:i.supportedActions}))}},"Manual reconciliation top cases");}catch(error){logger.error({err:error},"Manual reconciliation top cases failed");}}
async function logFocusedManualEvidence(){try{const q=await pool.query(`SELECT q.id AS queue_id,q.reason,q.candidate_client_id,er.external_id AS goldie_client_id,ecr.display_name,ecr.normalized_phone,ecr.email,ecr.secondary_phone,c.id AS candidate_id,c.display_name AS candidate_name,cc.value AS candidate_phone FROM client_reconciliation_queue q JOIN external_records er ON er.id=q.external_record_id JOIN external_client_records ecr ON ecr.external_record_id=er.id LEFT JOIN clients c ON c.id=q.candidate_client_id LEFT JOIN client_contacts cc ON cc.client_id=c.id AND cc.contact_type='phone' AND cc.is_primary=TRUE WHERE q.id IN (246,775,70,228,341) ORDER BY q.id`);logger.info({focusedManualEvidence:q.rows.map(r=>({queueId:String(r.queue_id),reason:r.reason,goldieClientId:r.goldie_client_id,displayName:r.display_name,normalizedPhone:r.normalized_phone,emailPresent:Boolean(r.email),secondaryPhonePresent:Boolean(r.secondary_phone),candidateClientId:r.candidate_id?String(r.candidate_id):null,candidateName:r.candidate_name||null,candidatePhoneMatches:Boolean(r.candidate_phone&&r.normalized_phone&&String(r.candidate_phone).replace(/\D/g,'').endsWith(String(r.normalized_phone).replace(/\D/g,'').slice(-9))) }))},"Focused manual reconciliation evidence");}catch(error){logger.error({err:error},"Focused manual reconciliation evidence failed");}}
async function runControlledStartupDataWork(){try{await runConfiguredCreateNewPromotion(logger);}catch(error){logger.error({err:error},"Configured Goldie create-new promotion failed");} await logCanonicalizationAuditStatus(); await logAppointmentIdentityEvidenceStatus(); await logSecondPassStatus(); await logTopManualQueue(); await logFocusedManualEvidence();}
