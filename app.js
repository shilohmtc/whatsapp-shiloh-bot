...
const { runBookingPolicySelfTest } = require("./src/services/bookingPolicySelfTest");
...
async function start() {
  server = app.listen(PORT, () => {
    ...
    if (String(process.env.RUN_BOOKING_POLICY_SELF_TEST_ON_STARTUP || "").toLowerCase() === "true") {
      setTimeout(async () => {
        try {
          const result = await runBookingPolicySelfTest();
          logger.info(
            { policyVersion: result.policyVersion, assertions: result.assertions },
            "BOOKING_POLICY_SELF_TEST_PASS"
          );
        } catch (error) {
          logger.error({ err: error }, "BOOKING_POLICY_SELF_TEST_FAIL");
        }
      }, 3000).unref();
    }
  });
