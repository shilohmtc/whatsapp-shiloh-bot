...
async function logDuplicateNameEvidence(){try{const q=await pool.query(`SELECT q.id AS queue_id,er.external_id,ecr.display_name,ecr.normalized_phone,ecr.email,ecr.secondary_phone,ecr.notes,er.source_payload FROM client_reconciliation_queue q JOIN external_records er ON er.id=q.external_record_id JOIN external_client_records ecr ON ecr.external_record_id=er.id WHERE q.id IN (199,698,723,726) ORDER BY ecr.display_name,q.id`);logger.info({duplicateNameEvidence:q.rows.map(r=>({queueId:String(r.queue_id),externalId:r.external_id,displayName:r.display_name,phoneFingerprint:r.normalized_phone?String(r.normalized_phone).slice(-4):null,emailPresent:Boolean(r.email),secondaryPhonePresent:Boolean(r.secondary_phone),notesPresent:Boolean(r.notes),rawFingerprint:require('crypto').createHash('sha256').update(JSON.stringify(r.source_payload||{})).digest('hex').slice(0,12)}))},"Duplicate-name reconciliation evidence");}catch(error){logger.error({err:error},"Duplicate-name reconciliation evidence failed");}}
