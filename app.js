...
async function applyConfiguredMigration(){
  const requested=process.env.APPLY_DATABASE_MIGRATIONS_ON_STARTUP;
  if(!requested) return;
  if(requested!=="007_historical_goldie_services.sql"){logger.error({requested},"Refusing unapproved startup migration request");return;}
  try{
    const before=await getMigrationStatus();
    const target=before.find(m=>m.filename===requested);
    if(!target){logger.error({requested},"Requested startup migration not found");return;}
    if(target.applied){logger.info({requested,checksumMatches:target.checksumMatches},"Requested startup migration already applied");return;}
    const otherPending=before.filter(m=>!m.applied&&m.filename!==requested);
    if(otherPending.length){logger.error({requested,otherPending:otherPending.map(m=>m.filename)},"Refusing migration because other pending migrations exist");return;}
    const result=await applyPendingMigrations();
    const after=await getMigrationStatus();
    const verified=after.find(m=>m.filename===requested);
    logger.info({requested,result,verified},"Guarded startup migration applied");
  }catch(error){logger.error({err:error,requested},"Guarded startup migration failed");}
}
...
async function logChantelDryRun(){try{const r=await buildChantelDuplicatePlan();logger.info({chantelDuplicateDryRun:{mode:r.mode,writesPerformed:r.writesPerformed,eligible:r.eligible,blockers:r.blockers,contactCollision:r.contactCollision,policy:{allowedQueueIds:r.policy.allowedQueueIds,requiresSameNormalizedName:r.policy.requiresSameNormalizedName,requiresSameNormalizedPhone:r.policy.requiresSameNormalizedPhone,requiresIdenticalSourcePayload:r.policy.requiresIdenticalSourcePayload,canonicalContactCollisionAllowed:r.policy.canonicalContactCollisionAllowed},pair:r.pair.map(p=>({queueId:p.queueId,displayName:p.displayName,hasEmail:p.hasEmail,hasSecondaryPhone:p.hasSecondaryPhone,payloadFingerprint:p.payloadFingerprint}))}},"Chantel duplicate dry-run plan");return r;}catch(error){logger.error({err:error},"Chantel duplicate dry-run plan failed");return null;}}
...
async function logMarindaDryRun(){try{const r=await buildMarindaPlan({clientBatchId:"1",appointmentBatchId:"2"});logger.info({marindaDryRun:{eligible:r.eligible,policy:r.policy,case:r.case?{queueId:r.case.queueId,displayName:r.case.displayName,reason:r.case.reason,exactAppointmentCount:r.case.exactAppointmentCount,emailPresent:r.case.emailPresent,secondaryPhonePresent:r.case.secondaryPhonePresent}:null,blockers:r.blockers}},"Marinda canonicalization dry-run plan");return r;}catch(error){logger.error({err:error},"Marinda canonicalization dry-run failed");return null;}}
