...
const { buildSeparateIdentityPlan } = require("./src/services/separateIdentityCanonicalization");
...
async function runControlledStartupDataWork(){try{await runConfiguredCreateNewPromotion(logger);}catch(error){logger.error({err:error},"Configured Goldie create-new promotion failed");} const dryRun=await logChantelDryRun(); if(dryRun?.eligible){try{await runConfiguredChantelDuplicateCanonicalization(logger);}catch(error){logger.error({err:error},"Configured Chantel duplicate canonicalization failed");}} await logCanonicalizationAuditStatus(); await logAppointmentIdentityEvidenceStatus(); await logSecondPassStatus(); await logTopManualQueue(); await logSeparateIdentityDryRun();}
